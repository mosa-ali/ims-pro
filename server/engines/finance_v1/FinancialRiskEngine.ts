...
export interface RiskAssessment {
  ...
  recommendations: string[];
  assessmentDate: Date;
  dimensionalBreakdown?: Record<string, { score: number; level: string }>;
}
...
export class FinancialRiskEngine {
  ...
  async assessFinancialRisk(
    ...
    const dimensionalBreakdown: Record<string, { score: number; level: string }> = {};
    dimensions.forEach(d => {
      dimensionalBreakdown[d.name] = {
        score: d.score,
        level: d.level,
      };
    });
  ...
  /**
   * Get historical risk assessments for trend analysis.
   */
  async getHistoricalRiskAssessments(
    organizationId: number,
    operatingUnitId?: number | null,
    months: number = 12
  ): Promise<RiskTrend[]> {
    // Generate historical trend data (placeholder - will be populated from database in future)
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      trends.push({
        period,
        riskScore: 50 + Math.random() * 30,  // Placeholder: 50-80 range
        riskLevel: 'medium',
        dimensionScores: {
          liquidity: 60,
          fx: 55,
