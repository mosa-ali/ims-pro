...
export async function getStatsData(ctx: any) {
  ...
  try {
    ...
    const budgetData = await db
      .select({
        totalBudget: sum(grants.totalBudget),
  ...
  try {
    ...
    const monthlyTrend = await db
      .select({
        ...
        approvedBudget: sum(grants.totalBudget),
...
export async function getFinancialAnalyticsData(ctx: any) {
  ...
  try {
    ...
    const grantBudgetData = await db
      ...
      .groupBy(grants.id, grants.grantName, grants.totalBudget);
    ...
    // Fetch cost pool distribution by sector (grants.sector)
    const costPoolData = await db
      .select({
        sector: grants.sector,
        amount: sum(payments.amount),
      })
      .from(grants)
      .leftJoin(payments, eq(grants.id, payments.grantId))
