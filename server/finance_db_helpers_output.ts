...
import {
  ...
  payments,
  glPostingEvents,
  glAccounts,          // Fix 7: was chartOfAccounts — that is the legacy table.
                       // gl_accounts + gl_account_categories is the live CoA system
                       // used by journalEntriesRouter and FinanceChartOfAccounts.tsx.
                       // chartOfAccounts is no longer imported here.
} from 'drizzle/schema';
...
/**
 * Guard: throw if the finance period covering `transactionDate` is locked.
 *
 * Call this at the start of any write path that posts to the ledger:
 *   - approveExpenditure
 *   - markExpenditurePaid
 *   - markPaymentPaid
 *   - any journal entry insert
 *
 * Fix 8: finance_periods table existed but was never enforced as a gate on
 * postings. This function closes that gap.
 */
export async function assertPeriodNotLocked(
  ...
  const [lockedPeriod] = await db
    .select({ id: financePeriods.id, periodName: financePeriods.periodCode })
    .from(financePeriods)
    .where(
      and(
        ...
        sql`DATE(${financePeriods.startDate}) <= ${date}`,
        sql`DATE(${financePeriods.endDate})   >= ${date}`,
      )
    )
    .limit(1);
  ...
  if (lockedPeriod) {
    throw new Error(
      `Finance period "${lockedPeriod.periodName}" is locked. ` +
      `Postings to ${date} are not permitted. ` +
      `Reopen the period in Financial Settings before proceeding.`
    );
  }
}
 ...
 * Blocks if the expenditure date falls in a locked finance period.
 */
export async function approveExpenditure(
  ...
  await db.transaction(async (tx) => {
    const [record] = await tx
      .select({
        id: financeExpenditures.id,
        status: financeExpenditures.status,
    ...
    // Fix 8a: block posting to locked periods
    if (record.expenditureDate) {
      await assertPeriodNotLocked(tx as unknown as DB, ctx, record.expenditureDate);
    }
...
export async function markExpenditurePaid(
  ...
  await db.transaction(async (tx) => {
    ...
    // Fix 8b: block posting to locked periods
    if (record.expenditureDate) {
      await assertPeriodNotLocked(tx as unknown as DB, ctx, record.expenditureDate);
    }
 ...
 * Blocks if the payment date falls in a locked finance period.
 */
export async function markPaymentPaid(
  ...
  await db.transaction(async (tx) => {
    ...
    // Fix 8c: block posting to locked periods
    if (record.paymentDate) {
      await assertPeriodNotLocked(tx as unknown as DB, ctx, record.paymentDate);
    }
