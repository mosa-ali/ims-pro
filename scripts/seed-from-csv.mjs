 ...
 * It handles multiple tables and ensures proper data mapping and validation.
 */
  ...
  try {
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      const values = batch.map((record) =>
        columns.map((col) => convertValue(record[col], col))
      );
    ...
    console.log(`  ✓ Inserted/Updated ${inserted} records in ${tableName}`);
    return inserted;
  } catch (error) {
...
async function seedDatabase() {
  ...
  try {
    ...
    console.log(`\n✅ Seeding complete! Total records inserted/updated: ${totalRecords}`);
