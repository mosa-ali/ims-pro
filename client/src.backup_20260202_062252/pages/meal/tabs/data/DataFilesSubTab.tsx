...
export function DataFilesSubTab({ survey }: Props) {
  ...
  const t = {
    filesManager: language === 'en' ? 'Files Manager' : 'مدير الملفات',
    syncToCloud: language === 'en' ? 'Sync to SharePoint / OneDrive' : 'مزامنة إلى SharePoint / OneDrive',
    syncAll: language === 'en' ? 'Sync All Files' : 'مزامنة جميع الملفات',
    syncing: language === 'en' ? 'Syncing...' : 'جاري المزامنة...',
    fileName: language === 'en' ? 'File Name' : 'اسم الملف',
    fileType: language === 'en' ? 'Type' : 'النوع',
    question: language === 'en' ? 'Question' : 'السؤال',
    submissionId: language === 'en' ? 'Submission ID' : 'معرف التقديم',
    submittedBy: language === 'en' ? 'Submitted By' : 'قُدّم بواسطة',
    date: language === 'en' ? 'Date' : 'التاريخ',
    size: language === 'en' ? 'Size' : 'الحجم',
    status: language === 'en' ? 'Status' : 'الحالة',
    actions: language === 'en' ? 'Actions' : 'الإجراءات',
    noFiles: language === 'en' ? 'No files available' : 'لا توجد ملفات متاحة',
    filesDesc: language === 'en' ? 'Files from survey submissions will appear here' : 'ستظهر الملفات من تقديمات المسح هنا',
    noFileQuestions: language === 'en' ? 'This survey has no file upload questions' : 'هذا المسح لا يحتوي على أسئلة تحميل ملفات',
    ...
    syncSuccess: language === 'en' ? 'Files synced successfully!' : 'تم مزامنة الملفات بنجاح!',
    folderPath: language === 'en' ? 'Cloud Folder' : 'مجلد السحابة',
    totalFiles: language === 'en' ? 'Total Files' : 'إجمالي الملفات',
    syncedFiles: language === 'en' ? 'Synced' : 'متزامنة',
    pendingFiles: language === 'en' ? 'Pending' : 'معلقة',
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-gray-50 rounded-lg border border-gray-200 p-4">
          <div className={`text-sm text-gray-700 mb-1 ${isRTL ? 'text-right' : 'text-left'}`}>
            {t.pendingFiles}
          </div>
          <div className={`text-2xl font-bold text-gray-900 ${isRTL ? 'text-right' : 'text-left'}`}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full" dir={isRTL ? 'rtl' : 'ltr'}>
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className={`px-4 py-3 text-xs font-medium text-gray-700 uppercase ${isRTL ? 'text-right' : 'text-left'}`}>
                    {t.fileName}
                  </th>
                  <th className={`px-4 py-3 text-xs font-medium text-gray-700 uppercase ${isRTL ? 'text-right' : 'text-left'}`}>
                    {t.fileType}
                  ...
                  <tr key={file.id} className="hover:bg-gray-50">
                    ...
                    <td className={`px-4 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                      <span className="text-sm text-gray-600">
                        {new Date(file.submittedAt).toLocaleDateString(language === 'en' ? 'en-US' : 'ar-EG')}
                      </span>
                    </td>
