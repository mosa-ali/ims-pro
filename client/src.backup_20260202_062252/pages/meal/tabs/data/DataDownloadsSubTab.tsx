...
export function DataDownloadsSubTab({ survey }: Props) {
  const { language, isRTL } = useLanguage();
  const [submissions, setSubmissions] = useState<Submission[]>([]);
  const [exportHistory, setExportHistory] = useState<ExportRecord[]>([]);
  const [exportType, setExportType] = useState('XLS');
  const [exportLanguage, setExportLanguage] = useState(language === 'en' ? 'English' : 'Arabic (ara)');

  const t = {
    downloads: language === 'en' ? 'Downloads' : 'التنزيلات',
    selectExportType: language === 'en' ? 'Select export type' : 'اختر نوع التصدير',
    xls: language === 'en' ? 'XLS' : 'XLS',
    csv: language === 'en' ? 'CSV' : 'CSV',
    pdf: language === 'en' ? 'PDF' : 'PDF',
    valueHeaderFormat: language === 'en' ? 'Value and header format' : 'تنسيق القيمة والعنوان',
    english: language === 'en' ? 'English' : 'الإنجليزية',
    arabic: language === 'en' ? 'Arabic (ara)' : 'العربية (ara)',
    advancedOptions: language === 'en' ? 'Advanced options' : 'خيارات متقدمة',
    applySettings: language === 'en' ? 'Apply saved export settings' : 'تطبيق إعدادات التصدير المحفوظة',
    noSettings: language === 'en' ? 'No export settings selected' : 'لم يتم تحديد إعدادات التصدير',
    export: language === 'en' ? 'Export' : 'تصدير',
    exports: language === 'en' ? 'Exports' : 'الصادرات',
    type: language === 'en' ? 'Type' : 'النوع',
    created: language === 'en' ? 'Created' : 'تم الإنشاء',
    languageLabel: language === 'en' ? 'Language' : 'اللغة',
    submissions: language === 'en' ? 'Submissions' : 'التقديمات',
    download: language === 'en' ? 'Download' : 'تنزيل',
    noData: language === 'en' ? 'No submissions to export' : 'لا توجد تقديمات للتصدير',
    noDataDesc: language === 'en' ? 'Submit survey responses to enable data exports' : 'قدم إجابات المسح لتفعيل تصدير البيانات',
    exportSuccess: language === 'en' ? 'Export created successfully!' : 'تم إنشاء التصدير بنجاح!',
    noExports: language === 'en' ? 'No export history' : 'لا يوجد سجل للتصديرات',
  };
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className={`px-6 py-3 font-semibold text-gray-700 ${isRTL ? 'text-right' : 'text-left'}`}>
                    {t.type}
                  </th>
                  <th className={`px-6 py-3 font-semibold text-gray-700 ${isRTL ? 'text-right' : 'text-left'}`}>
                    {t.created}
              ...
              <tbody className="divide-y divide-gray-200">
                {exportHistory.map((exp) => (
                  <tr key={exp.id} className="hover:bg-gray-50">
                    <td className={`px-6 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>{exp.type}</td>
                    <td className={`px-6 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                      {new Date(exp.created).toLocaleDateString(language === 'en' ? 'en-US' : 'ar-EG', {
                        month: 'long',
                        day: 'numeric',
                        year: 'numeric'
                      })}
                    </td>
                    <td className={`px-6 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>{exp.language}</td>
                    <td className={`px-6 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>{exp.submissionsCount}</td>
                    <td className={`px-6 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>
