...
export function DataGallerySubTab({ survey }: Props) {
  ...
  const t = {
    imageGallery: language === 'en' ? 'Image Gallery' : 'معرض الصور',
    syncToCloud: language === 'en' ? 'Sync to SharePoint / OneDrive' : 'مزامنة إلى SharePoint / OneDrive',
    syncAll: language === 'en' ? 'Sync All Images' : 'مزامنة جميع الصور',
    syncing: language === 'en' ? 'Syncing...' : 'جاري المزامنة...',
    from: language === 'en' ? 'From' : 'من',
    allQuestions: language === 'en' ? 'All questions' : 'جميع الأسئلة',
    between: language === 'en' ? 'Between' : 'بين',
    and: language === 'en' ? 'and' : 'و',
    noImages: language === 'en' ? 'No images available' : 'لا توجد صور متاحة',
    imagesDesc: language === 'en' ? 'Images from survey submissions will appear here' : 'ستظهر الصور من تقديمات المسح هنا',
    submittedBy: language === 'en' ? 'Submitted by' : 'قُدّم بواسطة',
    on: language === 'en' ? 'on' : 'في',
    synced: language === 'en' ? 'Synced' : 'متزامن',
    pending: language === 'en' ? 'Pending' : 'معلق',
    error: language === 'en' ? 'Error' : 'خطأ',
    notSynced: language === 'en' ? 'Not Synced' : 'غير متزامن',
    syncSuccess: language === 'en' ? 'Images synced successfully!' : 'تم مزامنة الصور بنجاح!',
    folderPath: language === 'en' ? 'Cloud Folder' : 'مجلد السحابة',
    totalImages: language === 'en' ? 'Total Images' : 'إجمالي الصور',
    syncedImages: language === 'en' ? 'Synced' : 'متزامنة',
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-12">
          <div className="text-center">
            ...
            <p className="text-gray-400 text-sm">{language === 'en' ? 'This survey has no photo questions' : 'هذا المسح لا يحتوي على أسئلة صورية'}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="space-y-4">
      ...
      <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${isRTL ? 'flex-row-reverse' : ''}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <div className={`text-sm text-gray-600 mb-1 ${isRTL ? 'text-right' : 'text-left'}`}>
            {t.totalImages}
          </div>
        ...
        <div className="bg-green-50 rounded-lg border border-green-200 p-4">
          <div className={`text-sm text-green-700 mb-1 ${isRTL ? 'text-right' : 'text-left'}`}>
            {t.syncedImages}
          </div>
          <div className={`text-2xl font-bold text-green-900 ${isRTL ? 'text-right' : 'text-left'}`}>
            {syncedCount}
          </div>
        </div>
        <div className="bg-blue-50 rounded-lg border border-blue-200 p-4">
          <div className={`text-xs text-blue-700 mb-1 ${isRTL ? 'text-right' : 'text-left'}`}>
            {t.folderPath}
          </div>
          <div className={`text-xs font-mono text-blue-900 break-all ${isRTL ? 'text-right' : 'text-left'}`}>
            {cloudFolderPath}
          </div>
        </div>
      </div>
              ...
              <div key={img.id} className="group relative">
                ...
                <div className="mt-2">
                  ...
                  <p className="text-xs text-gray-400">
                    {new Date(img.submittedAt).toLocaleDateString(language === 'en' ? 'en-US' : 'ar-EG')}
                  </p>
                </div>
              </div>
            ))}
            ...
            <div className={`flex items-center justify-between p-4 border-b border-gray-200 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className={isRTL ? 'text-right' : 'text-left'}>
                ...
                <p className="text-sm text-gray-600">
                  {t.submittedBy} {previewImage.submittedBy} • {new Date(previewImage.submittedAt).toLocaleDateString(language === 'en' ? 'en-US' : 'ar-EG')}
                </p>
              </div>
              <button
                onClick={() => setPreviewImage(null)}
            ...
            <div className="p-4">
              ...
              <div className={`mt-4 flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                {previewImage.cloudUrl && (
                  <span className="text-xs text-gray-500 font-mono ml-2">{previewImage.cloudUrl}</span>
                )}
              </div>
            </div>
            
            {/* Modal Footer */}
            <div className={`flex items-center gap-3 p-4 border-t border-gray-200 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <button
