...
export function DataReportsSubTab({ survey }: Props) {
  ...
  const t = {
    ...
    defaultReport: language === 'en' ? 'Default Report' : 'التقرير الافتراضي',
    warningMessage: language === 'en'
      ...
      : 'هذا تقرير آلي بناءً على البيانات الأولية المقدمة لهذا المشروع. يرجى إجراء تنظيف مناسب للبيانات قبل استخدام الرسوم البيانية والأرقام المستخدمة في هذه الصفحة.',
    selectQuestion: language === 'en' ? 'Select a question to analyze' : 'اختر سؤالاً للتحليل',
    noData: language === 'en' ? 'No data available' : 'لا تتوفر بيانات',
    noDataDesc: language === 'en' ? 'Submit survey responses to see analytics here' : 'قدم إجابات المسح لرؤية التحليلات هنا',
    respondents: language === 'en' ? 'respondents answered this question' : 'مستجيبون أجابوا على هذا السؤال',
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-700 text-white">
                <tr>
                  <th className={`px-4 py-2 font-semibold ${isRTL ? 'text-right' : 'text-left'}`}>{t.value}</th>
                  <th className={`px-4 py-2 font-semibold ${isRTL ? 'text-right' : 'text-left'}`}>{t.frequency}</th>
                  <th className={`px-4 py-2 font-semibold ${isRTL ? 'text-right' : 'text-left'}`}>{t.percentage}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {reportData.chartData.map((row: any, index: number) => (
                  <tr key={index} className="hover:bg-gray-50">
                    <td className={`px-4 py-2 ${isRTL ? 'text-right' : 'text-left'}`}>{row.value}</td>
                    <td className={`px-4 py-2 ${isRTL ? 'text-right' : 'text-left'}`}>{row.count}</td>
                    <td className={`px-4 py-2 ${isRTL ? 'text-right' : 'text-left'}`}>{row.percentage}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
