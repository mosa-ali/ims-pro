...
export function AttendanceCalendar() {
  ...
  const t = {
    title: language === 'en' ? 'Attendance Calendar' : 'تقويم الحضور',
    subtitle: language === 'en' ? 'Visual attendance tracking with day, week, and month views' : 'تتبع الحضور المرئي مع عرض اليوم والأسبوع والشهر',
    
    day: language === 'en' ? 'Day' : 'يوم',
    ...
    staff: language === 'en' ? 'Staff' : 'الموظف',
    status: language === 'en' ? 'Status' : 'الحالة',
    hours: language === 'en' ? 'Hours' : 'الساعات',
    overtime: language === 'en' ? 'Overtime' : 'عمل إضافي',
    
    present: language === 'en' ? 'Present' : 'حاضر',
    absent: language === 'en' ? 'Absent' : 'غائب',
    late: language === 'en' ? 'Late' : 'متأخر',
    onLeave: language === 'en' ? 'On Leave' : 'في إجازة',
    fieldWork: language === 'en' ? 'Field Work' : 'عمل ميداني',
    
    noRecords: language === 'en' ? 'No attendance records for this period' : 'لا توجد سجلات حضور لهذه الفترة',
    
    mondayShort: language === 'en' ? 'Mon' : 'الإثنين',
    tuesdayShort: language === 'en' ? 'Tue' : 'الثلاثاء',
    wednesdayShort: language === 'en' ? 'Wed' : 'الأربعاء',
    thursdayShort: language === 'en' ? 'Thu' : 'الخميس',
    fridayShort: language === 'en' ? 'Fri' : 'الجمعة',
    saturdayShort: language === 'en' ? 'Sat' : 'السبت',
    sundayShort: language === 'en' ? 'Sun' : 'الأحد'
  };
  ...
  const dayNames = [t.mondayShort, t.tuesdayShort, t.wednesdayShort, t.thursdayShort, t.fridayShort, t.saturdayShort, t.sundayShort];

  const getCurrentPeriodLabel = () => {
    if (viewMode === 'day') {
      return currentDate.toLocaleDateString(language === 'en' ? 'en-US' : 'ar-SA', { 
        weekday: 'long', 
        year: 'numeric', 
        month: 'long', 
        day: 'numeric' 
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="grid grid-cols-7 border-b border-gray-200">
          {weekDays.map((day, idx) => (
            <div key={idx} className={`p-4 text-center border-r border-gray-200 last:border-r-0 ${isRTL ? 'text-right' : 'text-left'}`}>
        ...
        <div className="grid grid-cols-7">
          {weekDays.map((day, idx) => {
            ...
            return (
              <div key={idx} className="border-r border-gray-200 last:border-r-0 p-2 min-h-[200px]">
                {dayRecords.map(record => (
                  <div
                    key={record.id}
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="grid grid-cols-7 border-b border-gray-200 bg-gray-50">
          {dayNames.map((day, idx) => (
            <div key={idx} className={`p-3 text-center font-semibold text-sm text-gray-700 ${isRTL ? 'text-right' : 'text-left'}`}>
              {day}
            </div>
          ))}
        </div>
          ...
          {days.map((day, idx) => {
            ...
            return (
              ...
              >
                ...
                {dayRecords.length > 3 && (
                  <p className="text-xs text-gray-500 mt-1">+{dayRecords.length - 3} {language === 'en' ? 'more' : 'المزيد'}</p>
                )}
              </div>
            );
          })}
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-auto">
            ...
            <div className={`p-6 space-y-4 ${isRTL ? 'text-right' : 'text-left'}`}>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-gray-600 mb-1">{language === 'en' ? 'Status' : 'الحالة'}</p>
                  <p className="font-semibold">{selectedRecord.status}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">{language === 'en' ? 'Source' : 'المصدر'}</p>
                  <p className="font-semibold">{getSourceBadge(selectedRecord.source)} {selectedRecord.source}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">{language === 'en' ? 'Planned Hours' : 'الساعات المخططة'}</p>
                  <p className="font-semibold">{selectedRecord.plannedHours}h</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">{language === 'en' ? 'Actual Hours' : 'الساعات الفعلية'}</p>
                  <p className="font-semibold">{selectedRecord.actualHours}h</p>
                </div>
                {selectedRecord.overtimeHours > 0 && (
                  <div>
                    <p className="text-sm text-gray-600 mb-1">{language === 'en' ? 'Overtime' : 'عمل إضافي'}</p>
              ...
              {selectedRecord.notes && (
                <div className="p-3 bg-gray-50 rounded-lg">
                  <p className="text-sm text-gray-600 mb-1">{language === 'en' ? 'Notes' : 'ملاحظات'}</p>
                  <p className="text-sm text-gray-900">{selectedRecord.notes}</p>
                </div>
              )}
            </div>

            <div className={`p-6 border-t border-gray-200 ${isRTL ? 'text-right' : 'text-left'}`}>
              <button
