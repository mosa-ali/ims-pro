...
export function ApprovalSection({ plan, isEditing, onUpdate }: ApprovalSectionProps) {
  const { language, isRTL } = useLanguage();

  const t = {
    title: language === 'en' ? 'Approval & Governance' : 'الاعتماد والحوكمة',
    description: language === 'en'
      ? 'Approval workflow and governance records for the HR Annual Plan'
      : 'سير عمل الاعتماد وسجلات الحوكمة للخطة السنوية للموارد البشرية',
    ...
    comments: language === 'en' ? 'Comments' : 'التعليقات',
    ...
    approvalWorkflow: language === 'en' ? 'Approval Workflow' : 'سير عمل الاعتماد',
    step1: language === 'en' ? 'Step 1: Preparation' : 'الخطوة 1: الإعداد',
    step2: language === 'en' ? 'Step 2: Review' : 'الخطوة 2: المراجعة',
    step3: language === 'en' ? 'Step 3: Approval' : 'الخطوة 3: الاعتماد',
    step4: language === 'en' ? 'Step 4: Locked' : 'الخطوة 4: القفل',
    
    completed: language === 'en' ? 'Completed' : 'مكتمل',
    pending: language === 'en' ? 'Pending' : 'معلق',
    
    governanceNotes: language === 'en' ? 'Governance Notes' : 'ملاحظات الحوكمة',
    governanceText: language === 'en'
      ? 'Once approved, this plan becomes read-only and serves as the authoritative reference for recruitment, budgeting, and capacity building throughout the planning year. All changes must be documented through formal amendment procedures.'
      : 'بمجرد الموافقة، تصبح هذه الخطة للقراءة فقط وتعمل كمرجع موثوق للتوظيف والميزانية وبناء القدرات طوال سنة التخطيط. يجب توثيق جميع التغييرات من خلال إجراءات التعديل الرسمية.'
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="relative">
          {/* Progress Line */}
          <div className="absolute top-5 left-0 right-0 h-1 bg-gray-200" style={{ left: '2.5rem', right: '2.5rem' }}></div>
          <div 
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h3 className={`text-lg font-semibold text-gray-900 mb-4 ${isRTL ? 'text-right' : 'text-left'}`}>
          {language === 'en' ? 'Approval Details' : 'تفاصيل الاعتماد'}
        </h3>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h4 className={`text-sm font-semibold text-blue-900 mb-2 ${isRTL ? 'text-right' : 'text-left'}`}>
          {t.governanceNotes}
        </h4>
