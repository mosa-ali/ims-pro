...
export function MasterDataSection({
  ...
  const t = {
    ...
    departments: language === 'en' ? 'Departments' : 'الأقسام',
    positions: language === 'en' ? 'Positions' : 'المناصب',
    contractTypes: language === 'en' ? 'Contract Types' : 'أنواع العقود',
    leaveTypes: language === 'en' ? 'Leave Types' : 'أنواع الإجازات',
    exitReasons: language === 'en' ? 'Exit Reasons' : 'أسباب المغادرة',
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-2">{t.masterData}</h3>
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <div className="bg-gray-50 border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-gray-900">{t.departments}</h4>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t.active}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t.createdDate}</th>
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <div className="bg-gray-50 border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-gray-900">{t.positions}</h4>
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {positions.map(pos => (
                <tr key={pos.positionId} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm">
                    <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                      pos.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                    }`}>
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <div className="bg-gray-50 border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-gray-900">{t.contractTypes}</h4>
          ...
          {contractTypes.map(ct => (
            <div key={ct.contractTypeId} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                ...
                <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                  ct.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                }`}>
                  {ct.active ? t.active : t.inactive}
                </span>
              </div>
            </div>
          ))}
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <div className="bg-gray-50 border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-gray-900">{t.leaveTypes}</h4>
          ...
          {leaveTypes.map(lt => (
            <div key={lt.leaveTypeId} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                  lt.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                }`}>
                  {lt.active ? t.active : t.inactive}
                </span>
              </div>
              <p className="text-xs text-gray-500">
                {t.deductibleFromAnnual}: {lt.deductibleFromAnnual ? '✓' : '✗'}
              </p>
            </div>
          ))}
        </div>
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        <div className="bg-gray-50 border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-gray-900">{t.exitReasons}</h4>
          ...
          {exitReasons.map(er => (
            <div key={er.reasonId} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                ...
                <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                  er.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                }`}>
                  {er.active ? t.active : t.inactive}
                </span>
              </div>
            </div>
          ))}
