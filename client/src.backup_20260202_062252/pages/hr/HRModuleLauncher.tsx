...
export function HRModuleLauncher() {
  ...
  const t = {
    ...
    subtitle: language === 'en' 
      ? 'Staff registry, payroll, leave, and contract management' 
      : 'سجل الموظفين، الرواتب، الإجازات، وإدارة العقود',
    
    // Status badges
    active: language === 'en' ? 'Active' : 'نشط',
    notImplemented: language === 'en' ? 'Not Yet Implemented' : 'لم يُنفذ بعد',
    restricted: language === 'en' ? 'Restricted Access' : 'وصول محظور',
    ...
    noAccess: language === 'en' ? 'You do not have access to this module' : 'ليس لديك صلاحية الوصول',
  };
      ...
      case 'not-implemented':
        return (
          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700 border border-yellow-200">
            ⏳ {module.badge ? module.badge[language] : t.notImplemented}
          </span>
        );
      case 'restricted':
        return (
  ...
  return (
    <div className="space-y-6">
        ...
        {modules.map((module) => {
          ...
          return (
            ...
            >
              ...
              {/* Active indicator */}
              {module.status === 'active' && (
                <div className={`mt-4 pt-4 border-t border-gray-100 flex items-center gap-2 text-sm text-blue-600 font-medium ${isRTL ? 'flex-row-reverse' : ''}`}>
      ...
      {/* Footer Info */}
      <div className={`mt-8 p-4 bg-blue-50 border border-blue-200 rounded-lg ${isRTL ? 'text-right' : 'text-left'}`}>
