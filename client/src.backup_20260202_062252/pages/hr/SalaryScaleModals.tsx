...
export function EditSalaryModal({ record, grades: propGrades, language = 'en', isRTL = false, onClose, onSave, userName = 'System' }: EditSalaryModalProps) {
  ...
  const t = {
    title: language === 'en' ? 'Edit Salary Record' : 'تعديل سجل الراتب',
    staffInfo: language === 'en' ? 'Staff Information' : 'معلومات الموظف',
    gradeScale: language === 'en' ? 'Grade & Scale' : 'الدرجة والمستوى',
    grade: language === 'en' ? 'Grade' : 'الدرجة',
    step: language === 'en' ? 'Step' : 'المستوى',
    salaryRange: language === 'en' ? 'Salary Range' : 'نطاق الراتب',
    minSalary: language === 'en' ? 'Min' : 'الأدى',
    maxSalary: language === 'en' ? 'Max' : 'الأقصى',
    approvedSalary: language === 'en' ? 'Approved Gross Salary' : 'الراتب الإجمالي المعتمد',
    allowances: language === 'en' ? 'Allowances' : 'البدلات',
    housing: language === 'en' ? 'Housing Allowance' : 'بدل السكن',
    transport: language === 'en' ? 'Transport Allowance' : 'بدل المواصلات',
    representation: language === 'en' ? 'Representation Allowance' : 'بدل التمثيل',
    annual: language === 'en' ? 'Annual Allowance' : 'البدل السنوي',
    bonus: language === 'en' ? 'Bonus' : 'المكافأة',
    other: language === 'en' ? 'Other Allowances' : 'بدلات أخرى',
    effectiveDate: language === 'en' ? 'Effective Start Date' : 'تاريخ السريان',
    value: language === 'en' ? 'Value' : 'قيمة',
    percentage: language === 'en' ? 'Percentage' : 'نسبة',
    cancel: language === 'en' ? 'Cancel' : 'إلغاء',
    save: language === 'en' ? 'Save as New Version' : 'حفظ كنسخة جديدة',
    versionNote: language === 'en' 
      ? 'Saving will create a new version and mark the current record as superseded'
      : 'الحفظ سينشئ نسخة جديدة ويضع علامة على السجل الحالي كمستبدل'
  };
        ...
        <div className="px-6 py-4 space-y-6">
          ...
          <div className="bg-gray-50 p-4 rounded">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-gray-600">{language === 'en' ? 'Name' : 'الاسم'}:</span> <span className="font-medium">{record.staffFullName}</span></div>
              <div><span className="text-gray-600">{language === 'en' ? 'Position' : 'المنصب'}:</span> <span className="font-medium">{record.position}</span></div>
              <div><span className="text-gray-600">{language === 'en' ? 'Department' : 'القسم'}:</span> <span className="font-medium">{record.department}</span></div>
            </div>
          </div>
                ...
                return (
                  <div key={item.key}>
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{language === 'en' ? 'Type' : 'النوع'}</label>
                        <select
                          value={allowanceType}
                    ...
                    {allowanceType === 'percentage' && formData.approvedGrossSalary > 0 && (
                      <div className="mt-1 text-sm text-blue-600 bg-blue-50 px-2 py-1 rounded">
                        {allowanceValue}% {language === 'en' ? 'of' : 'من'} ${formData.approvedGrossSalary.toLocaleString()} = ${calculatedValue.toLocaleString()}
                      </div>
                    )}
                  </div>
                );
...
export function SalaryHistoryModal({ staffId, staffName, language, isRTL, onClose }: HistoryModalProps) {
  const history = salaryScaleService.getByStaffId(staffId);

  const t = {
    title: language === 'en' ? 'Salary History' : 'سجل الراتب',
    for: language === 'en' ? 'for' : 'لـ',
    ...
    grade: language === 'en' ? 'Grade' : 'الدرجة',
    step: language === 'en' ? 'Step' : 'المستوى',
    salary: language === 'en' ? 'Approved Salary' : 'الراتب المعتمد',
    status: language === 'en' ? 'Status' : 'الحالة',
    approvedBy: language === 'en' ? 'Approved By' : 'اعتمد من',
    ...
    noHistory: language === 'en' ? 'No history found' : 'لا يوجد سجل'
  };
              ...
              {history.map((record, index) => (
                <div
                  ...
                  className={`border rounded-lg p-4 ${
                    record.status === 'active' ? 'border-green-300 bg-green-50' : 'border-gray-200'
                  }`}
                >
                  ...
                  <div className="grid grid-cols-4 gap-4 text-sm">
                    <div>
                      ...
                      <span className="font-medium ml-2">{record.grade}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">{t.step}:</span>
                      <span className="font-medium ml-2">{record.step}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">{t.salary}:</span>
                      <span className="font-medium ml-2">${record.approvedGrossSalary.toLocaleString()}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">{t.approvedBy}:</span>
                      <span className="font-medium ml-2">{record.lastApprovedBy || '-'}</span>
                    </div>
                  </div>
                </div>
              ))}
...
export function AddGradeModal({ language, isRTL, onClose, onSave }: AddGradeModalProps) {
  const [formData, setFormData] = useState({
    grade: '',
    description: '',
    minSalary: 0,
  ...
  const t = {
    title: language === 'en' ? 'Add New Grade' : 'إضافة درجة جديدة',
    grade: language === 'en' ? 'Grade Code' : 'رمز الدرجة',
    description: language === 'en' ? 'Description' : 'الوصف',
    minSalary: language === 'en' ? 'Minimum Salary' : 'الحد الأدنى للراتب',
    maxSalary: language === 'en' ? 'Maximum Salary' : 'الحد الأقصى للراتب',
    currency: language === 'en' ? 'Currency' : '��لعملة',
    cancel: language === 'en' ? 'Cancel' : 'إلغاء',
    save: language === 'en' ? 'Add Grade' : 'إضافة الدرجة',
    placeholder: {
      grade: language === 'en' ? 'e.g., G6' : 'مثال: G6',
      description: language === 'en' ? 'e.g., Executive Level' : 'مثال: مستوى تنفيذي'
    }
  };

  const handleSave = () => {
    if (!formData.grade || !formData.description) {
      alert(language === 'en' ? 'Please fill all required fields' : 'الرجاء ملء جميع الحقول المطلوبة');
      return;
    }
...
export function ManageGradesModal({ language, isRTL, onClose, onUpdate }: ManageGradesModalProps) {
  ...
  const t = {
    title: language === 'en' ? 'Manage Salary Grades' : 'إدارة درجات الرواتب',
    subtitle: language === 'en' ? 'View and delete grade definitions' : 'عرض وحذف تعريفات الدرجات',
    grade: language === 'en' ? 'Grade' : 'الدرجة',
    description: language === 'en' ? 'Description' : 'الوصف',
    salaryRange: language === 'en' ? 'Salary Range' : 'نطاق الراتب',
    steps: language === 'en' ? 'Steps' : 'المستويات',
    ...
    noGrades: language === 'en' ? 'No grades found' : 'لا توجد درجات'
  };
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">{t.grade}</th>
                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">{t.description}</th>
                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">{t.salaryRange}</th>
                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">{t.steps}</th>
