...
export function LeaveRequestForm({ employee, language, isRTL, onClose, onSave, editingRequest }: Props) {
  ...
  const t = {
    title: language === 'en' ? 'Leave Request' : 'طلب إجازة',
    employeeInfo: language === 'en' ? 'Employee Information' : 'معلومات الموظف',
    leaveDetails: language === 'en' ? 'Leave Details' : 'تفاصيل الإجازة',
    leaveBalance: language === 'en' ? 'Leave Balance' : 'رصيد الإجازة',
    ...
    annualLeave: language === 'en' ? 'Annual Leave' : 'إجازة سنوية',
    emergencyLeave: language === 'en' ? 'Emergency Leave' : 'إجازة طارئة',
    sickLeave: language === 'en' ? 'Sick Leave' : 'إجازة مرضية',
    otherLeave: language === 'en' ? 'Other Leave' : 'إجازة أخرى',
    
    startDate: language === 'en' ? 'Start Date' : 'تاريخ البدء',
    endDate: language === 'en' ? 'End Date' : 'تاريخ الانتهاء',
    totalDays: language === 'en' ? 'Total Leave Days' : 'إجمالي أيام الإجازة',
    reason: language === 'en' ? 'Reason' : 'السبب',
    justification: language === 'en' ? 'Justification (Required for Emergency > 3 days)' : 'المبرر (مطلوب للإجازة الطارئة > 3 أيام)',
    medicalReport: language === 'en' ? 'Medical Report (Required for Sick > 3 days)' : 'التقرير الطبي (مطلوب للإجازة المرضية > 3 أيام)',
    
    openingBalance: language === 'en' ? 'Opening Balance' : 'الرصيد الافتتاحي',
    usedLeave: language === 'en' ? 'Used Leave' : 'الإجازة المستخدمة',
    pendingLeave: language === 'en' ? 'Pending Leave' : 'الإجازة المعلقة',
    remainingBalance: language === 'en' ? 'Remaining Balance' : 'الرصيد المتبقي',
    availableBalance: language === 'en' ? 'Available Balance' : 'الرصيد المتاح',
    
    saveAsDraft: language === 'en' ? 'Save as Draft' : 'حفظ كمسودة',
    submit: language === 'en' ? 'Submit Request' : 'إرسال الطلب',
    cancel: language === 'en' ? 'Cancel' : 'إلغاء',
    
    uploadFile: language === 'en' ? 'Upload PDF' : 'تحميل PDF',
    fileUploaded: language === 'en' ? 'File Uploaded' : 'تم تحميل الملف',
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  placeholder={language === 'en' ? 'Enter reason for leave...' : 'أدخل سبب الإجازة...'}
                />
              </div>
              ...
              {formData.leaveType === 'Emergency Leave' && formData.totalDays > 3 && (
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={language === 'en' ? 'Provide detailed justification...' : 'قدم مبررًا مفصلاً...'}
                  />
                </div>
              )}
