...
export function LeaveRequestPrint({ request, language, isRTL, onClose }: Props) {
  ...
  const t = {
    title: language === 'en' ? 'LEAVE REQUEST FORM' : 'نموذج طلب الإجازة',
    requestNo: language === 'en' ? 'Request No.' : 'رقم الطلب',
    date: language === 'en' ? 'Date' : 'التاريخ',
    
    printButton: language === 'en' ? 'Print / Save as PDF' : 'طباعة / حفظ كـ PDF',
    ...
    contractDate: language === 'en' ? 'Contract Start Date' : 'تاريخ بدء العقد',
    ...
    // Balance Info
    balanceInfo: language === 'en' ? 'LEAVE BALANCE SUMMARY' : 'ملخص رصيد الإجازة',
    annualEntitlement: language === 'en' ? 'Annual Entitlement' : 'الاستحقاق السنوي',
    usedDays: language === 'en' ? 'Used Days' : 'الأيام المستخدمة',
    remainingBalance: language === 'en' ? 'Remaining Balance' : 'الرصيد المتبقي',
    days: language === 'en' ? 'days' : 'يوم',
    
    // Approval Section
    approvalSection: language === 'en' ? 'APPROVAL WORKFLOW' : 'سير العمل للموافقة',
    lineManager: language === 'en' ? 'Line Manager' : 'المدير المباشر',
    hrManager: language === 'en' ? 'HR Manager' : 'مدير الموارد البشرية',
    signature: language === 'en' ? 'Signature' : 'التوقيع',
    approvalDate: language === 'en' ? 'Date' : 'التاريخ',
    status: language === 'en' ? 'Status' : 'الحالة',
    approvedBy: language === 'en' ? 'Approved By' : 'تمت الموافقة بواسطة',
    approvalDateValue: language === 'en' ? 'Approval Date' : 'تاريخ الموافقة',
    
    // Footer
    notes: language === 'en' ? 'NOTES' : 'ملاحظات',
    printedOn: language === 'en' ? 'Printed on' : 'طُبع في',
    systemGenerated: language === 'en' ? 'This is a system-generated document' : 'هذه وثيقة صادرة عن النظام',
    
    // Medical Report
    medicalReport: language === 'en' ? 'Medical Report Attached' : 'التقرير الطبي مرفق',
    ...
    <div className="fixed inset-0 bg-gray-900/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
          <div className="print-container" dir={isRTL ? 'rtl' : 'ltr'}>
            ...
            <div className="print-section">
              ...
              {request.status === 'Rejected' && request.rejectedBy && (
                <div style={{ 
                  ...
                  <div className="print-grid">
                    <div className="print-field">
                      <div className="print-label">{language === 'en' ? 'Rejected By' : 'رُفض بواسطة'}</div>
                      <div className="print-value">{request.rejectedBy}</div>
                    </div>
                    <div className="print-field">
                      <div className="print-label">{language === 'en' ? 'Rejection Date' : 'تاريخ الرفض'}</div>
                      <div className="print-value">{formatDate(request.rejectedAt)}</div>
                    </div>
                  </div>
                  {request.rejectionReason && (
                    <div className="print-field" style={{ marginTop: '10px' }}>
                      <div className="print-label">{language === 'en' ? 'Rejection Reason' : 'سبب الرفض'}</div>
            ...
            <div className="print-footer">
              ...
              <p style={{ margin: '5px 0' }}>
                {t.printedOn}: {new Date().toLocaleDateString(language === 'ar' ? 'ar' : 'en', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                  hour: '2-digit',
