...
export function TrainingDevelopmentCard({ employee, language, isRTL }: Props) {
  ...
  const t = {
    title: language === 'en' ? 'Training & Development' : 'التدريب والتطوير',
    subtitle: language === 'en' ? 'Training programs and certifications' : 'برامج التدريب والشهادات',
    addTraining: language === 'en' ? 'Add Training' : 'إضافة تدريب',
    noTrainings: language === 'en' ? 'No training records yet' : 'لا توجد سجلات تدريب بعد',
    
    // Table headers
    trainingTitle: language === 'en' ? 'Training Title' : 'عنوان التدريب',
    type: language === 'en' ? 'Type' : 'النوع',
    provider: language === 'en' ? 'Provider' : 'الجهة المنظمة',
    ...
    // Status labels
    planned: language === 'en' ? 'Planned' : 'مخطط',
    ongoing: language === 'en' ? 'Ongoing' : 'جاري',
    completed: language === 'en' ? 'Completed' : 'مكتمل',
    cancelled: language === 'en' ? 'Cancelled' : 'ملغي',
    
    // Certificate status
    issued: language === 'en' ? 'Issued' : 'صادر',
    notIssued: language === 'en' ? 'Not Issued' : 'غير صادر',
    
    // Actions
    edit: language === 'en' ? 'Edit' : 'تعديل',
    delete: language === 'en' ? 'Delete' : 'حذف',
    deleteConfirm: language === 'en' ? 'Are you sure you want to delete this training record?' : 'هل أنت متأكد من حذف سجل التدريب هذا؟',
    
    hours: language === 'en' ? 'hours' : 'ساعة'
  };
            ...
            <table className="w-full" dir={isRTL ? 'rtl' : 'ltr'}>
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-xs font-semibold text-gray-700 text-right">{t.cost}</th>
