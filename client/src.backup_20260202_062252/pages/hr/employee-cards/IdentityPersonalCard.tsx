...
export function IdentityPersonalCard({ employee, language, isRTL, onEmployeeUpdate }: Props) {
  ...
  const t = {
    title: language === 'en' ? 'Identity & Personal Profile' : 'الهوية والبيانات الشخصية',
    subtitle: language === 'en' ? 'Core employee information and contact details' : 'المعلومات الأساسية للموظف وتفاصيل الاتصال',
      ...
      <div className="p-6 space-y-6">
        ...
        <div className={`p-4 bg-gray-50 rounded-lg ${isRTL ? 'text-right' : 'text-left'}`}>
          ...
          <p className="text-sm text-gray-900">
            {employee.address || (language === 'en' ? 'No address provided' : 'لم يتم توفير عنوان')}
          </p>
        </div>
        ...
        <div className={`p-4 bg-blue-50 border border-blue-200 rounded-lg ${isRTL ? 'text-right' : 'text-left'}`}>
          ...
          <p className="text-sm text-blue-900 font-medium">
            {employee.supervisor || (language === 'en' ? 'No supervisor assigned' : 'لم يتم تعيين مشرف')}
          </p>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {/* Identification */}
            <div className={`p-3 border border-gray-200 rounded-lg ${isRTL ? 'text-right' : 'text-left'}`}>
              <p className="text-xs text-gray-500 mb-1">{t.identification}</p>
              <p className="text-sm text-gray-400">
                {language === 'en' ? 'No file uploaded' : 'لم يتم رفع ملف'}
              </p>
            </div>
