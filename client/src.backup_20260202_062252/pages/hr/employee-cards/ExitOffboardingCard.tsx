...
export function ExitOffboardingCard({ employee, language, isRTL, onEmployeeUpdate }: Props) {
  ...
  const t = {
    title: language === 'en' ? 'Exit & Offboarding' : 'الخروج والإنهاء',
    subtitle: language === 'en' ? 'Exit process and clearance' : 'عملية الروج والتصفية',
    initiateExit: language === 'en' ? 'Initiate Exit Process' : 'بدء عملية الخروج',
    viewDetails: language === 'en' ? 'View Details' : 'عرض التفاصيل',
    updateClearance: language === 'en' ? 'Update Clearance' : 'تحديث التصفية',
    noExit: language === 'en' ? 'No exit process initiated' : 'لم تبدأ عملية الخروج',
    employeeActive: language === 'en' ? 'Employee is currently active' : 'الموظف نشط حالياً',
    exitInProgress: language === 'en' ? 'Exit process in progress' : 'عملية الخروج جارية',
    completionRate: language === 'en' ? 'Completion' : 'نسبة الإنجاز',
    
    // Exit info
    exitType: language === 'en' ? 'Exit Type' : 'نوع الخروج',
    lastWorkingDay: language === 'en' ? 'Last Working Day' : 'آخر يوم عمل',
    status: language === 'en' ? 'Status' : 'الحالة',
    
    // Checklist
    clearanceChecklist: language === 'en' ? 'Clearance Checklist' : 'قائمة التصفية',
    hrClearance: language === 'en' ? 'HR Clearance' : 'تصفية الموارد البشرية',
    financeClearance: language === 'en' ? 'Finance Clearance' : 'تصفية المالية',
    itClearance: language === 'en' ? 'IT Clearance' : 'تصفية تقنية المعلومات',
    adminClearance: language === 'en' ? 'Admin Clearance' : 'الصفية الإدارية',
    finalSettlement: language === 'en' ? 'Final Settlement Paid' : 'دفع التسوية النهائية',
    experienceCertificate: language === 'en' ? 'Experience Certificate Issued' : 'إصدار شهادة خبرة',
    exitInterview: language === 'en' ? 'Exit Interview Conducted' : 'إجراء مقابلة الخروج',
    
    completed: language === 'en' ? 'Completed' : 'مكتمل',
    pending: language === 'en' ? 'Pending' : 'معلق',
    
    // Status
    initiated: language === 'en' ? 'Initiated' : 'مبدأ',
    inProgress: language === 'en' ? 'In Progress' : 'جاري',
    pendingClearance: language === 'en' ? 'Pending Clearance' : 'في انتظار التصفية',
    exitCompleted: language === 'en' ? 'Exit Completed' : 'اكتمل الخروج'
  };
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className={`flex items-center gap-2 px-3 py-2 text-sm bg-orange-600 text-white rounded-lg hover:bg-orange-700 ${isRTL ? 'flex-row-reverse' : ''}`}
          >
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-3 py-2 text-sm bg-purple-600 text-white rounded-lg hover:bg-purple-700 ${isRTL ? 'flex-row-reverse' : ''}`}
          >
...
function ChecklistItem({ label, completed, date, by, language }: {
  label: string;
  completed: boolean;
  date?: string;
  by?: string;
    ...
    <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
      ...
      {completed && date && (
        <div className="text-xs text-gray-500">
          {new Date(date).toLocaleDateString(language === 'ar' ? 'ar' : 'en')}
          {by && ` • ${by}`}
        </div>
      )}
    </div>
