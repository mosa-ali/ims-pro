...
export function SanctionsDisciplinaryCard({ employee, language, isRTL }: Props) {
  ...
  const t = {
    ...
    subtitle: language === 'en' 
      ? 'Final approved disciplinary records from Sanctions Module' 
      : 'السجلات التأديبية النهائية المعتمدة من وحدة العقوبات',
    
    noRecords: language === 'en' ? 'No disciplinary records' : 'لا توجد سجلات تأديبية',
    cleanRecord: language === 'en' ? 'Clean disciplinary record - No sanctions on file' : 'سجل انضباطي نظيف - لا توجد عقوبات مسجلة',
    
    caseReference: language === 'en' ? 'Case Reference' : 'مرجع القضية',
    allegation: language === 'en' ? 'Allegation' : 'الادعاء',
    allegationDate: language === 'en' ? 'Allegation Date' : 'تاريخ الادعاء',
    finalDecision: language === 'en' ? 'Final Decision' : 'القرار النهائي',
    disciplinaryAction: language === 'en' ? 'Disciplinary Action' : 'الإجراء التأديبي',
    effectiveDate: language === 'en' ? 'Effective Date' : 'تاريخ السريان',
    approvedBy: language === 'en' ? 'Approved By' : 'اعتمد من قبل',
    approvalDate: language === 'en' ? 'Approval Date' : 'تاريخ الاعتماد',
    status: language === 'en' ? 'Status' : 'الحالة',
    
    generateForm: language === 'en' ? 'Generate Official Form' : 'إنشاء النموذج الرسمي',
    uploadSigned: language === 'en' ? 'Upload Signed Form' : 'تحميل النموذج الموقع',
    downloadSigned: language === 'en' ? 'Download Signed Form' : 'تنزيل النموذج الموقع',
    viewCase: language === 'en' ? 'View Full Case' : 'عرض القضية الكاملة',
    
    signedDocument: language === 'en' ? 'Signed Document' : 'مستند موقع',
    uploaded: language === 'en' ? 'Uploaded' : 'تم التحميل',
    uploadedOn: language === 'en' ? 'Uploaded on' : 'تم التحميل في',
    ...
    noteText: language === 'en'
      ? 'All disciplinary cases are managed through the Sanctions & Disciplinary Module. This tab shows final approved decisions for personnel file documentation only.'
      : 'تتم إدارة جميع القضايا التأديبية من خلال وحدة العقوبات والإجراءات التأديبية. تعرض هذه الصفحة القرارات النهائية المعتمدة لتوثيق ملف الموظف فقط.',
    
    pdfOnly: language === 'en' ? 'Only PDF files are allowed' : 'يُسمح فقط بملفات PDF',
    fileTooLarge: language === 'en' ? 'File size must be less than 5MB' : 'يجب أن يكون حجم الملف أقل من 5 ميجابايت',
    uploadSuccess: language === 'en' ? 'Signed form uploaded successfully' : 'تم تحميل النموذج الموقع بنجاح',
    
    totalCases: language === 'en' ? 'Total Cases' : 'إجمالي القضايا'
  };
        ...
        <div className={`flex items-start justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
          <div className={isRTL ? 'text-right' : 'text-left'}>
            <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              {cases.length > 0 && (
                <div className={`flex items-center gap-2 px-3 py-1 bg-red-50 rounded-lg border border-red-200 ${isRTL ? 'flex-row-reverse' : ''}`}>
      ...
      <div className="px-6 pt-4">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className={`flex items-start gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={isRTL ? 'text-right' : 'text-left'}>
              <h4 className="text-sm font-semibold text-blue-900">{t.importantNote}</h4>
      ...
      <div className="p-6">
        {cases.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-lg font-medium text-green-700 mb-1">{t.noRecords}</p>
            <p className="text-sm text-gray-600">{t.cleanRecord}</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                    ...
                    <div className={isRTL ? 'text-right' : 'text-left'}>
                      <div className={`flex items-center gap-2 mb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        ...
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${
                          disciplinaryCase.status === 'Approved' 
                            ? 'bg-green-100 text-green-700 border-green-200'
                            : 'bg-gray-100 text-gray-700 border-gray-200'
                        }`}>
                          {disciplinaryCase.status === 'Approved' ? t.approved : t.closed}
                        </span>
                      </div>
                      <p className="text-xs text-gray-500">
                        {t.approvalDate}: {formatDate(disciplinaryCase.approvalDate)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 mb-4">
                    {/* Allegation */}
                    <div className={isRTL ? 'text-right' : 'text-left'}>
                      <p className="text-xs font-semibold text-gray-600 mb-1">{t.allegation}:</p>
                      <p className="text-sm text-gray-900">{disciplinaryCase.allegationDetails}</p>
                      <p className="text-xs text-gray-500 mt-1">
                        {t.allegationDate}: {formatDate(disciplinaryCase.allegationDate)}
                      </p>
                    </div>

                    {/* Final Decision */}
                    {disciplinaryCase.finalDecision && (
                      <div className={`bg-yellow-50 border border-yellow-200 rounded-lg p-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                        <p className="text-xs font-semibold text-yellow-900 mb-1">{t.finalDecision}:</p>
                    ...
                    {disciplinaryCase.disciplinaryAction && (
                      <div className={`bg-red-50 border border-red-200 rounded-lg p-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                        <p className="text-xs font-semibold text-red-900 mb-1">{t.disciplinaryAction}:</p>
                        <p className="text-sm text-red-800 font-medium">{disciplinaryCase.disciplinaryAction}</p>
                        {disciplinaryCase.effectiveDate && (
                          <p className="text-xs text-red-700 mt-2">
                            {t.effectiveDate}: {formatDate(disciplinaryCase.effectiveDate)}
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    {disciplinaryCase.approvedBy && (
                      <div className={`text-xs text-gray-600 ${isRTL ? 'text-right' : 'text-left'}`}>
                        <p>
                          <span className="font-semibold">{t.approvedBy}:</span> {disciplinaryCase.approvedBy}
                  ...
                  {signedDoc && (
                    <div className={`bg-green-50 border border-green-200 rounded-lg p-3 mb-4 ${isRTL ? 'text-right' : 'text-left'}`}>
                      <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                        <CheckCircle className="w-4 h-4 text-green-600" />
                        <span className="text-sm font-semibold text-green-900">{t.signedDocument}: {signedDoc.fileName}</span>
                      </div>
                      <p className="text-xs text-green-700 mt-1">
                        {t.uploadedOn}: {formatDate(signedDoc.uploadedDate)}
                      </p>
                    </div>
                  )}
                  ...
                  <div className={`flex items-center gap-3 flex-wrap ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-2 px-4 py-2 text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 ${isRTL ? 'flex-row-reverse' : ''}`}
                    >
                      <Printer className="w-4 h-4" />
                      <span>{t.generateForm}</span>
                    </button>
                    ...
                    {!signedDoc && (
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 text-sm font-medium text-green-700 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 ${isRTL ? 'flex-row-reverse' : ''}`}
                      >
                        <Upload className="w-4 h-4" />
                        <span>{t.uploadSigned}</span>
                      </button>
                    )}
...
function OfficialDisciplinaryFormPrintModal({ 
  ...
  const t = {
    title: language === 'en' ? 'Official Disciplinary Form' : 'نموذج العقوبة التأديبية الرسمي',
    organizationName: language === 'en' ? 'Humanitarian Organization' : 'المنظمة الإنسانية',
    confidential: language === 'en' ? 'CONFIDENTIAL - Personnel File' : 'سري - ملف الموظف',
    ...
    allegationDetails: language === 'en' ? 'Allegation Details' : 'تفاصيل الادعاء',
    allegationDate: language === 'en' ? 'Allegation Date' : 'تاريخ الادعاء',
    
    finalDecision: language === 'en' ? 'Final Decision' : 'القرار النهائي',
    disciplinaryAction: language === 'en' ? 'Disciplinary Action to be Taken' : 'الإجراء التأديبي المتخذ',
    effectiveDate: language === 'en' ? 'Effective Date' : 'تاريخ السريان',
    
    approvalSection: language === 'en' ? 'Approval' : 'الاعتماد',
    approvedBy: language === 'en' ? 'Approved By' : 'اعتمد من قبل',
    approverPosition: language === 'en' ? 'Position' : 'الوظيفة',
    approvalDate: language === 'en' ? 'Approval Date' : 'تاريخ الاعتماد',
    signature: language === 'en' ? 'Signature' : 'التوقيع',
    
    employeeAcknowledgment: language === 'en' ? 'Employee Acknowledgment' : 'إقرار الموظف',
    acknowledgmentText: language === 'en'
      ? 'I acknowledge that I have received and reviewed this disciplinary action. I understand the contents and the effective date.'
      : 'أقر بأنني قد استلمت ومراجعة هذا الإجراء التأديبي. أفهم المحتوى وتاريخ السريان.',
    ...
    printButton: language === 'en' ? 'Print Form' : 'طباعة النموذج',
    closeButton: language === 'en' ? 'Close' : 'إغلاق'
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        {/* Print Content */}
        <div ref={contentRef} className="p-8 print:p-12" style={{ direction: isRTL ? 'rtl' : 'ltr' }}>
          {/* Organization Header */}
          <div className="text-center mb-8 border-b-2 border-gray-300 pb-6">
            ...
            <p className="text-sm text-red-600 font-medium">{t.confidential}</p>
          </div>

          {/* Case Reference */}
          <div className="mb-6 bg-gray-100 p-4 rounded">
            <p className="text-sm font-semibold text-gray-700">{t.caseReference}:</p>
            <p className="text-lg font-mono font-bold text-gray-900">{disciplinaryCase.caseReference}</p>
          </div>

          {/* Employee Information */}
          <div className="mb-6">
            <h3 className="text-lg font-bold text-gray-900 mb-3 border-b border-gray-300 pb-2">{t.employeeInfo}</h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-600">{t.staffId}:</p>
          ...
          {/* Allegation Details */}
          <div className="mb-6">
            <h3 className="text-lg font-bold text-gray-900 mb-3 border-b border-gray-300 pb-2">{t.allegationDetails}</h3>
            <p className="text-base text-gray-900 mb-3">{disciplinaryCase.allegationDetails}</p>
            <p className="text-sm text-gray-600">
              <span className="font-semibold">{t.allegationDate}:</span> {formatDate(disciplinaryCase.allegationDate)}
            </p>
          </div>
          ...
          {disciplinaryCase.finalDecision && (
            <div className="mb-6 bg-yellow-50 border-2 border-yellow-300 p-4 rounded">
              <h3 className="text-lg font-bold text-yellow-900 mb-3">{t.finalDecision}</h3>
          ...
          {disciplinaryCase.disciplinaryAction && (
            <div className="mb-6 bg-red-50 border-2 border-red-300 p-4 rounded">
              <h3 className="text-lg font-bold text-red-900 mb-3">{t.disciplinaryAction}</h3>
              <p className="text-base text-red-900 font-medium whitespace-pre-wrap">
                {disciplinaryCase.disciplinaryAction}
              </p>
              {disciplinaryCase.effectiveDate && (
                <p className="text-sm text-red-800 mt-3">
                  <span className="font-semibold">{t.effectiveDate}:</span> {formatDate(disciplinaryCase.effectiveDate)}
                </p>
              )}
            </div>
          )}
          ...
          {/* Approval Section */}
          <div className="mb-8">
            <h3 className="text-lg font-bold text-gray-900 mb-3 border-b border-gray-300 pb-2">{t.approvalSection}</h3>
            <div className="grid grid-cols-2 gap-6">
              <div>
                <p className="text-sm text-gray-600 mb-1">{t.approvedBy}:</p>
                <p className="text-base font-semibold text-gray-900">{disciplinaryCase.approvedBy || '-'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600 mb-1">{t.approverPosition}:</p>
                <p className="text-base font-semibold text-gray-900">{disciplinaryCase.approverPosition || '-'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600 mb-1">{t.approvalDate}:</p>
                <p className="text-base font-semibold text-gray-900">{formatDate(disciplinaryCase.approvalDate)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600 mb-1">{t.signature}:</p>
          ...
          <div className="border-2 border-gray-300 p-6 rounded">
            <h3 className="text-lg font-bold text-gray-900 mb-3">{t.employeeAcknowledgment}</h3>
            <p className="text-sm text-gray-700 mb-6">{t.acknowledgmentText}</p>
            
            <div className="grid grid-cols-2 gap-6">
              <div>
                <p className="text-sm text-gray-600 mb-1">{t.employeeSignature}:</p>
          ...
          <div className="mt-8 pt-4 border-t border-gray-300 text-center text-xs text-gray-500">
            ...
            <p className="mt-1">{t.organizationName} - {t.confidential}</p>
          </div>
        </div>
      </div>
    </div>
