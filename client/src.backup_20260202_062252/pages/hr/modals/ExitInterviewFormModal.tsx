...
export function ExitInterviewFormModal({ employee, onClose, onSave }: Props) {
  ...
  const t = {
    title: language === 'en' ? 'Exit Interview Notes' : 'ملاحظات مقابلة الخروج',
    subtitle: language === 'en' ? 'Structured Exit Feedback' : 'ملاحظات الخروج المنظمة',
    ...
    interviewDate: language === 'en' ? 'Interview Date' : 'تاريخ المقابلة',
    interviewedBy: language === 'en' ? 'Interviewed By' : 'تمت المقابلة من قبل',
    interviewedByRole: language === 'en' ? 'Role / Position' : 'الدور / المنصب',
    
    exitType: language === 'en' ? 'Exit Type' : 'نوع الخروج',
    resignation: language === 'en' ? 'Resignation' : 'استقالة',
    endOfContract: language === 'en' ? 'End of Contract' : 'انتهاء العقد',
    termination: language === 'en' ? 'Termination' : 'فصل',
    
    feedback: language === 'en' ? 'Exit Feedback' : 'ملاحظات الخروج',
    primaryReasonForLeaving: language === 'en' ? 'Primary Reason for Leaving' : 'السبب الرئيسي للمغادرة',
    whatWorkedWell: language === 'en' ? 'What Worked Well?' : 'ما الذي كان يعمل بشكل جيد؟',
    areasForImprovement: language === 'en' ? 'Areas for Improvement' : 'مجالات التحسين',
    wouldReturn: language === 'en' ? 'Would you consider returning?' : 'هل تفكر في العودة؟',
    additionalComments: language === 'en' ? 'Additional Comments' : 'تعليقات إضافية',
    
    ratings: language === 'en' ? 'Experience Ratings (1-5)' : 'تقييمات التجربة (1-5)',
    workEnvironment: language === 'en' ? 'Work Environment' : 'بيئة العمل',
    management: language === 'en' ? 'Management' : 'الإدارة',
    compensation: language === 'en' ? 'Compensation' : 'التعويض',
    careerDevelopment: language === 'en' ? 'Career Development' : 'التطوير المهني',
    
    notes: language === 'en' ? 'Interviewer Notes' : 'ملاحظات المقابل',
    ...
    success: language === 'en' ? 'Exit interview saved successfully' : 'تم حفظ مقابلة الخروج بنجاح'
  };
    ...
    <ModalOverlay onClose={onClose}>
      <div className="bg-white rounded-lg w-full max-w-3xl max-h-[90vh] flex flex-col" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className={`flex items-center gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <button
            onClick={handleSave}
            className={`flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 ${isRTL ? 'flex-row-reverse' : ''}`}
          >
