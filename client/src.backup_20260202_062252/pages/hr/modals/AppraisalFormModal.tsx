...
export function AppraisalFormModal({ employee, onClose, onSave }: Props) {
  ...
  const t = {
    ...
    subtitle: language === 'en' ? 'Annual / Periodic Performance Review' : 'تقييم الأداء السنوي / الدوري',
    ...
    reviewYear: language === 'en' ? 'Review Year' : 'سنة التقييم',
    reviewQuarter: language === 'en' ? 'Quarter (Optional)' : 'الربع (اختياري)',
    
    // Reviewer info
    reviewerInfo: language === 'en' ? 'Reviewer Information' : 'معلومات المقيّم',
    reviewerName: language === 'en' ? 'Reviewer Name' : 'اسم المقيّم',
    reviewerPosition: language === 'en' ? 'Reviewer Position' : 'منصب المقيّم',
    reviewDate: language === 'en' ? 'Review Date' : 'تاريخ التقييم',
    
    // Performance rating
    performanceRating: language === 'en' ? 'Performance Rating' : 'تقييم الأداء',
    overallRating: language === 'en' ? 'Overall Rating (1-5)' : 'التقييم الإجمالي (1-5)',
    ratingDescription: language === 'en' ? 'Rating Description' : 'وصف التقييم',
    
    // Ratings
    excellent: language === 'en' ? 'Excellent' : 'ممتاز',
    good: language === 'en' ? 'Good' : 'جيد',
    satisfactory: language === 'en' ? 'Satisfactory' : 'مرضي',
    needsImprovement: language === 'en' ? 'Needs Improvement' : 'يحتاج تحسين',
    unsatisfactory: language === 'en' ? 'Unsatisfactory' : 'غير مرضي',
    ...
    // Recommendation
    recommendation: language === 'en' ? 'Final Recommendation' : 'التوصية النهائية',
    continue: language === 'en' ? 'Continue' : 'الاستمرار',
    promote: language === 'en' ? 'Promote' : 'ترقية',
    trainingRequired: language === 'en' ? 'Training Required' : 'يحتاج تدريب',
    pip: language === 'en' ? 'Performance Improvement Plan' : 'خطة تحسين الأداء',
    
    // Acknowledgement
    employeeAcknowledged: language === 'en' ? 'Employee Acknowledgement' : 'إقرار الموظف',
    acknowledgeText: language === 'en' 
      ? 'I acknowledge that I have reviewed and discussed this appraisal' 
      : 'أقر بأنني راجعت ومناقشت هذا التقييم',
    ...
    success: language === 'en' ? 'Appraisal saved successfully' : 'تم حفظ التقييم بنجاح'
  };
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <span className="ml-2 font-medium text-gray-900">{employee.staffId}</span>
                </div>
                <div>
                  <span className="text-gray-600">{t.fullName}:</span>
                  <span className="ml-2 font-medium text-gray-900">{employee.fullName}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="ml-2 font-medium text-gray-900">{employee.department}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder={language === 'en' ? 'e.g., Annual 2024, Q1 2024' : 'مثال: السنوي 2024، الربع الأول 2024'}
