...
import { useLanguage, formatNumber } from '@/app/contexts/LanguageContext';
...
// Mock budget items from Financial Overview
const mockBudgetItems = [
  {
    id: 'budget-1',
    budgetCode: 'BL-1000',
    subBL: '1.1',
    budgetItem: 'Personnel Costs',
    category: 'Staff Costs',
    quantity: 1,
    unitType: 'person',
    unitCost: 4167,
    recurrence: 12,
    totalBudgetLine: 50000,
    currency: 'USD'
  },
  {
    id: 'budget-2',
    budgetCode: 'BL-2000',
    subBL: '2.1',
    budgetItem: 'Travel & Accommodation',
    category: 'Direct Costs',
    quantity: 10,
    unitType: 'trips',
    unitCost: 1500,
    recurrence: 1,
    totalBudgetLine: 15000,
    currency: 'USD'
  }
];
...
// Mock expenses
const mockExpenses = [
  { budgetItemId: 'budget-1', amount: 12500, date: '2025-03-15' },
  { budgetItemId: 'budget-2', amount: 4200, date: '2025-02-10' }
];
...
export function ForecastPlanTab({ projectId }: ForecastPlanTabProps) {
  ...
  useEffect(() => {
    ...
    const fiscalYears = generateFiscalYears(project.startDate, project.endDate);
    
    // Generate forecast rows: Each budget item × Each fiscal year
    const initializedForecasts: ForecastRow[] = [];
    
    mockBudgetItems.forEach(budgetItem => {
      fiscalYears.forEach(fy => {
        // Calculate actual spent for this year
        const yearStart = parseInt(fy.year.replace('FY', ''));
        const actualSpent = mockExpenses
          .filter(e => {
            const expenseDate = new Date(e.date);
            return e.budgetItemId === budgetItem.id && 
                   expenseDate.getFullYear() === yearStart;
          })
        ...
        if (fy.yearNumber > 1) {
          // Calculate cumulative actual spent up to end of previous year
          const previousYearEnd = new Date(yearStart - 1, 11, 31); // 31 Dec of previous year
          const cumulativeActualSpent = mockExpenses
            .filter(e => {
              const expenseDate = new Date(e.date);
              return e.budgetItemId === budgetItem.id && 
                     expenseDate <= previousYearEnd;
            })
            .reduce((sum, e) => sum + e.amount, 0);
          
          // Remaining balance = Original Budget - All spending up to 31 Dec Year N-1
          previousYearBudgetBalance = budgetItem.totalBudgetLine - cumulativeActualSpent;
        }
        ...
        initializedForecasts.push({
          ...
          subBL: budgetItem.subBL,
          activityName: budgetItem.budgetItem,
          category: budgetItem.category,
          quantity: budgetItem.quantity,
          unitType: budgetItem.unitType,
          unitCost: budgetItem.unitCost,
          recurrence: budgetItem.recurrence,
          totalBudgetLine: budgetItem.totalBudgetLine,
    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} className="space-y-6">
      ...
      <div className={`flex items-center justify-between mt-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
        <div className={isRTL ? 'text-right' : 'text-left'}>
          ...
          <p className="text-xs text-gray-600 mt-0.5">
            {isRTL 
              ? `شبكة التنبؤات متعددة السنوات - مزامنة تلقائية من النظرة المالية (${project.startDate} إلى ${project.endDate})`
              : `Multi-year forecast grid - Auto-synced from Financial Overview (${project.startDate} to ${project.endDate})`
            }
          </p>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
        <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={`text-sm text-blue-900 ${isRTL ? 'text-right' : 'text-left'}`}>
            {isRTL ? (
              <>
                <strong>فترة المشروع:</strong> {project.startDate} إلى {project.endDate} 
              ...
              <>
                <strong>Project Period:</strong> {project.startDate} to {project.endDate} 
                <span className="mx-2">•</span>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ul className={`text-sm text-blue-700 space-y-1 ${isRTL ? 'text-right' : 'text-left'}`}>
          {isRTL ? (
            <>
              <li>✅ <strong>دعم السنوات المتعددة:</strong> توليد تلقائي للسنوات المالية من تواريخ المشروع ({project.startDate} إلى {project.endDate})</li>
            ...
            <>
              <li>✅ <strong>Multi-Year Support:</strong> Auto-generates fiscal years from project dates ({project.startDate} to {project.endDate})</li>
