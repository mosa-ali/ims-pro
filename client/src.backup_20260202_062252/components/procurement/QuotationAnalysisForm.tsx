...
export function QuotationAnalysisForm({ analysisId, onUpdate, language }: Props) {
    ...
    <div className="space-y-8 print:space-y-4 print:p-0">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="bg-blue-50 px-6 py-4 border-b border-blue-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700 font-medium">
                {language === 'ar' ? 'تحليل العروض' : 'Quotation Analysis Form'}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {language === 'ar' ? 'طباعة' : 'Print Official QA'}
            </button>
          </div>
        </div>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-4 gap-6 bg-white">
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{language === 'ar' ? 'رقم طلب الشراء' : 'PR Number'}</label>
            <p className="text-sm font-bold text-gray-900 bg-gray-50 p-2 rounded border border-gray-200">{analysis.prNumber}</p>
          </div>
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{language === 'ar' ? 'المشروع' : 'Project Name'}</label>
            <p className="text-sm font-medium text-gray-900 bg-gray-50 p-2 rounded border border-gray-200">{pr.projectName || 'N/A'}</p>
          </div>
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{language === 'ar' ? 'المانح / المنحة' : 'Donor / Grant'}</label>
            <p className="text-sm font-medium text-gray-900 bg-gray-50 p-2 rounded border border-gray-200">{pr.donorName || 'N/A'}</p>
          </div>
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{language === 'ar' ? 'بند الميزانية' : 'Budget Line'}</label>
            <p className="text-sm font-medium text-gray-900 bg-gray-50 p-2 rounded border border-gray-200">{pr.budgetLineName}</p>
          </div>
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{language === 'ar' ? 'العملة' : 'Currency'}</label>
            <p className="text-sm font-medium text-gray-900 bg-gray-50 p-2 rounded border border-gray-200">{pr.currency}</p>
          </div>
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{language === 'ar' ? 'إجمالي طلب الشراء' : 'Total PR Amount'}</label>
          ...
          <div className="space-y-1 md:col-span-2">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{language === 'ar' ? 'نوع العملية' : 'Process Type'}</label>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider">
            {language === 'ar' ? 'مصفوفة عروض الموردين' : 'Supplier Offer Matrix'}
          </h3>
        </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-50/50">
                <th key="col-h" className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase border-b border-gray-200 w-16">#</th>
                <th key="col-desc" className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase border-b border-gray-200 min-w-[250px]">{language === 'ar' ? 'وصف الصنف' : 'Item Description'}</th>
                <th key="col-qty" className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase border-b border-gray-200 w-24">{language === 'ar' ? 'الكمية' : 'Qty'}</th>
                <th key="col-unit" className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase border-b border-gray-200 w-24">{language === 'ar' ? 'الوحدة' : 'Unit'}</th>
            ...
            <tbody className="divide-y divide-gray-200">
                      ...
                      <td key={supplier.supplierId} className="px-6 py-4 bg-blue-50/20">
                        <div className="space-y-1">
                          <div className="relative">
                            <span className="absolute left-2 top-1.5 text-xs text-gray-400">{pr.currency}</span>
                            <input
                              type="number"
                              ...
                              disabled={isReadOnly}
                              placeholder="0.00"
                              className="w-full pl-10 pr-3 py-1.5 text-sm font-bold text-right border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-50 disabled:text-gray-500"
                            />
                          </div>
              ...
              <tr className="bg-gray-100/50">
                <td colSpan={4} className="px-6 py-4 text-right text-xs font-bold text-gray-900 uppercase tracking-wider">
                  {language === 'ar' ? 'إجمالي العرض' : 'SUPPLIER SUB-TOTAL'}
                </td>
                {analysis.suppliers.map((supplier) => (
                  <td key={supplier.supplierId} className="px-6 py-4 text-center">
                    <span className={`text-sm font-black p-2 rounded ${
                      supplier.subtotal === analysis.lowestQuotedAmount && supplier.subtotal > 0
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider">
              {language === 'ar' ? 'ملخص التقييم' : 'Evaluation & Scoring Summary'}
            </h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50/30">
                  <th key="h-supplier" className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase">{language === 'ar' ? 'المورد' : 'Supplier'}</th>
                  <th key="h-tech" className="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase">{language === 'ar' ? 'التقني' : 'Technical'}</th>
                  <th key="h-fin" className="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase">{language === 'ar' ? 'المالي' : 'Financial'}</th>
                  <th key="h-total" className="px-6 py-4 text-center text-xs font-bold text-white uppercase bg-gray-900">{language === 'ar' ? 'الإجمالي' : 'Total Score'}</th>
                  <th key="h-rank" className="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase">{language === 'ar' ? 'الترتيب' : 'Rank'}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                    ...
                    <tr key={supplier.supplierId} className={`${supplier.isBestValue ? 'bg-green-50/30' : ''}`}>
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${
                          supplier.rank === 1 ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-600'
                        }`}>
                          {supplier.rank || '-'}
                        </span>
                      </td>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
          <div className="bg-green-600 px-6 py-4 border-b border-green-700 flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white uppercase tracking-wider">
              {language === 'ar' ? 'اختيار المورد' : 'Final Selection'}
            </h3>
          </div>
          <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
            {analysis.suppliers.find(s => s.isBestValue) ? (
              <div className="bg-green-50 border border-green-100 p-4 rounded-lg">
                <p className="text-xs font-bold text-green-700 uppercase mb-1">{language === 'ar' ? 'أفضل قيمة مقابل المال' : 'Best Evaluated Supplier'}</p>
              ...
              <div className="bg-yellow-50 border border-yellow-100 p-4 rounded-lg flex items-center gap-3">
                ...
                <p className="text-xs font-bold text-yellow-700">{language === 'ar' ? 'يرجى إدخال كافة العروض والتقييمات' : 'Please complete all pricing and technical scoring to identify best value.'}</p>
              </div>
            )}

            <div className="space-y-4 pt-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{language === 'ar' ? 'المورد المختار' : 'Selected Supplier'}</label>
                <select 
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{language === 'ar' ? 'مبررات الاختيار' : 'Selection Justification'}</label>
                <textarea 
                  className="w-full p-2.5 bg-gray-50 border border-gray-300 rounded-lg text-sm min-h-[100px] focus:ring-2 focus:ring-blue-500"
                  placeholder="Mandatory justification for supplier selection..."
      ...
      {errors.length > 0 && (
        <div className="bg-red-50 border border-red-200 p-4 rounded-xl flex items-start gap-3">
          ...
          <div className="space-y-1">
            <h4 className="text-sm font-bold text-red-900 uppercase tracking-wider">{language === 'ar' ? 'أخطاء التحقق' : 'Blocking Validation Errors'}</h4>
      ...
      <div className="hidden print:grid grid-cols-3 gap-8 mt-12 pt-8 border-t border-gray-200 bg-white">
        <div className="text-center space-y-4">
          <p className="text-xs font-bold text-gray-500 uppercase">Prepared By (Logistics)</p>
        ...
        <div className="text-center space-y-4">
          <p className="text-xs font-bold text-gray-500 uppercase">Reviewed By (Finance)</p>
          <div className="h-16 border-b border-gray-300 mx-auto w-40"></div>
          <p className="text-sm font-bold text-gray-900">Name: _________________</p>
          <p className="text-xs text-gray-500">Date: ____/____/2026</p>
        </div>
        <div className="text-center space-y-4">
          <p className="text-xs font-bold text-gray-500 uppercase">Approved By (Director)</p>
          <div className="h-16 border-b border-gray-300 mx-auto w-40"></div>
          <p className="text-sm font-bold text-gray-900">Name: _________________</p>
          <p className="text-xs text-gray-500">Date: ____/____/2026</p>
        </div>
