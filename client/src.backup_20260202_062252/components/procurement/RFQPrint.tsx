...
export function RFQPrint({ rfq, language, isRTL }: Props) {
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} className="print-document">
      {/* PRINT HEADER */}
      <div className="mb-6">
        ...
        {/* Document Info Grid */}
        <div className={`grid grid-cols-2 gap-4 text-sm pt-3 border-t-2 border-black ${isRTL ? 'text-right' : 'text-left'}`}>
          <div>
      ...
      {/* META SECTION */}
      <div className="mb-6">
        <div className={`grid grid-cols-2 gap-x-8 gap-y-3 text-sm ${isRTL ? 'text-right' : 'text-left'}`}>
          <div>
      ...
      <div className="mb-6">
        ...
        <table className="w-full text-xs border-collapse border border-black" dir={isRTL ? 'rtl' : 'ltr'}>
          <thead>
            <tr className="bg-gray-100">
              <th className={`px-2 py-2 border border-black font-bold ${isRTL ? 'text-right' : 'text-left'}`} style={{ width: '5%' }}>
                {trans.itemNo}
              </th>
              <th className={`px-2 py-2 border border-black font-bold ${isRTL ? 'text-right' : 'text-left'}`} style={{ width: '30%' }}>
                {trans.description}
              </th>
              <th className={`px-2 py-2 border border-black font-bold ${isRTL ? 'text-right' : 'text-left'}`} style={{ width: '35%' }}>
                {trans.specification}
              </th>
            ...
            {rfq.items && rfq.items.length > 0 ? (
              rfq.items.map((item, index) => (
                <tr key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  <td className={`px-2 py-2 border border-black ${isRTL ? 'text-right' : 'text-left'}`}>
                    {index + 1}
                  </td>
                  <td className={`px-2 py-2 border border-black ${isRTL ? 'text-right' : 'text-left'}`}>
                    {item.itemName}
        ...
        <table className="w-full text-xs border-collapse border border-black" dir={isRTL ? 'rtl' : 'ltr'}>
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className={`px-2 py-2 border border-black font-bold ${isRTL ? 'text-right' : 'text-left'}`} style={{ width: '25%' }}>
                {trans.supplierName}
              </th>
              <th className={`px-2 py-2 border border-black font-bold ${isRTL ? 'text-right' : 'text-left'}`} style={{ width: '20%' }}>
