...
export function CompetitiveBidAnalysisForm({ pr }: Props) {
  ...
  const translations = {
    title: language === 'en' ? 'Competitive Bid Analysis (CBA)' : 'تحليل العطاءات التنافسية',
    cbaNumber: language === 'en' ? 'CBA Number' : 'رقم CBA',
    prNumber: language === 'en' ? 'PR Number' : 'رقم طلب الشراء',
    tenderNumber: language === 'en' ? 'Tender / RFQ Number' : 'رقم العطاء',
    date: language === 'en' ? 'Date' : 'التاريخ',
    country: language === 'en' ? 'Country' : 'الدولة',
    currency: language === 'en' ? 'Currency' : 'العملة',
    budgetLine: language === 'en' ? 'Budget Line' : 'بند الميزانية',
    budgetAmount: language === 'en' ? 'Budget Amount' : 'مبلغ الميزانية',
    
    technicalEvaluation: language === 'en' ? 'Technical Evaluation (50 Points)' : 'التقييم الفني (50 نقطة)',
    criteria: language === 'en' ? 'Criteria' : 'المعايير',
    maxScore: language === 'en' ? 'Max Score' : 'الدرجة القصوى',
    totalTechnical: language === 'en' ? 'Total Technical' : 'إجمالي الفني',
    percentage: language === 'en' ? 'Percentage' : 'النسبة المئوية',
    qualified: language === 'en' ? 'Qualified' : 'مؤهل',
    notQualified: language === 'en' ? 'Not Qualified' : 'غير مؤهل',
    threshold: language === 'en' ? 'Technical Threshold: 70%' : 'الحد الأدنى الفني: 70٪',
    
    financialEvaluation: language === 'en' ? 'Financial Evaluation (50 Points)' : 'التقييم المالي (50 نقطة)',
    bidder: language === 'en' ? 'Bidder' : 'المتقدم',
    offeredPrice: language === 'en' ? 'Offered Price' : 'السعر المعروض',
    financialScore: language === 'en' ? 'Financial Score' : 'الدرجة المالية',
    
    finalScore: language === 'en' ? 'Final Score (100 Points)' : 'الدرجة النهائية (100 نقطة)',
    technical: language === 'en' ? 'Technical (50)' : 'فني (50)',
    financial: language === 'en' ? 'Financial (50)' : 'مالي (50)',
    total: language === 'en' ? 'Total (100)' : 'الإجمالي (100)',
    rank: language === 'en' ? 'Rank' : 'الترتيب',
    
    decision: language === 'en' ? 'Decision & Justification' : 'القرار والتبرير',
    lowestBidAmount: language === 'en' ? 'Lowest Bid Amount' : 'أقل مبلغ عطاء',
    selectedSupplier: language === 'en' ? 'Selected Supplier' : 'المورد المختار',
    selectionJustification: language === 'en' ? 'Justification (MANDATORY if not lowest bidder)' : 'التبرير (إلزامي إذا لم يكن أقل متقدم)',
    
    approvalSignatures: language === 'en' ? 'Approval & Signatures' : 'الموافقة والتوقيعات',
    committeeMember: language === 'en' ? 'Evaluation Committee Member' : 'عضو لجنة التقييم',
    ...
    syncFromCriteria: language === 'en' ? 'Sync from Criteria' : 'مزامنة من المعايير',
    save: language === 'en' ? 'Save' : 'حفظ',
    print: language === 'en' ? 'Print' : 'طباعة',
    complete: language === 'en' ? 'Complete CBA' : 'إكمال CBA',
    approve: language === 'en' ? 'Approve & Lock' : 'اعتماد وقفل',
    
    status: language === 'en' ? 'Status' : 'الحالة',
    draft: language === 'en' ? 'Draft' : 'مسودة',
    inEvaluation: language === 'en' ? 'In Evaluation' : 'قيد التقييم',
    completed: language === 'en' ? 'Completed' : 'مكتمل',
    approved: language === 'en' ? 'Approved' : 'معتمد',
    locked: language === 'en' ? 'Locked' : 'مقفل',
    
    noBidders: language === 'en' ? 'No bidders found. Please add bidders in Evaluation Criteria first.' : 'لم يتم العثور على متقدمين. يرجى إضافة المتقدمين في معايير التقييم أولاً.',
    justificationRequired: language === 'en' ? 'Justification is required when selected supplier is not the lowest bidder.' : 'التبرير مطلوب عند اختيار مورد غير أقل متقدم.'
  };
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="mt-4">
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${
            cba.status === 'draft' ? 'bg-gray-100 text-gray-800' :
            cba.status === 'in_evaluation' ? 'bg-blue-100 text-blue-800' :
            cba.status === 'completed' ? 'bg-green-100 text-green-800' :
                      ...
                      return (
                        <td
                          ...
                          className={`px-4 py-3 text-center font-bold border-r border-gray-300 ${
                            techScore?.qualifiedForFinancial ? 'text-green-900 bg-green-50' : 'text-red-900 bg-red-50'
                          }`}
                        >
                    ...
                    return (
                      <tr key={bidder.id} className={`border-b border-gray-200 ${!isQualified ? 'bg-gray-50' : ''}`}>
                        <td className="px-4 py-3 text-sm font-medium text-gray-900 border-r border-gray-200">
                          {bidder.companyName}
                          {!isQualified && (
                            <span className="ml-2 text-xs text-red-600">(Not Qualified)</span>
                          )}
                        </td>
                        <td className="px-4 py-3 border-r border-gray-200">
                          <input
                            ...
                            className="w-full px-3 py-2 text-right border border-gray-300 rounded focus:ring-2 focus:ring-green-500"
                          />
                        </td>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm font-medium text-gray-900 border-r border-gray-200">
                          {bidder?.companyName}
                          {score.rank === 1 && (
                            <span className="ml-2 text-xs text-yellow-700 font-semibold">HIGHEST SCORE</span>
                          )}
                        </td>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                placeholder="Provide detailed justification for supplier selection..."
              />
            </div>
