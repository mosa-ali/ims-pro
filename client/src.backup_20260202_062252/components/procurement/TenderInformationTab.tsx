...
export function TenderInformationTab({ pr, language, onUpdate }: Props) {
  const { user } = useAuth();
  ...
  const handleDeleteBidder = (bidderId: string) => {
    if (confirm('Delete this bidder entry?')) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">
                {language === 'ar' ? 'تفاصيل إعلان المناقصة' : 'Tender Announcement Details'}
              </h3>
              <p className="text-[10px] text-blue-100 font-medium">Public Tender - PR &gt; USD 25,000</p>
            </div>
          </div>
          {isExpired && (
            <div className="bg-white/10 px-3 py-1 rounded-full flex items-center gap-2">
              <Clock className="w-3 h-3 text-white" />
              <span className="text-[10px] font-bold text-white uppercase">Announcement Closed</span>
            </div>
          )}
        </div>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1">
              ...
              {language === 'ar' ? 'قناة الإعلان' : 'Announcement Channel'}
            </label>
            <select 
            ...
            >
              ...
              <option value="donor_portal">Donor Portal</option>
          ...
          <div className="md:col-span-2 space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1">
              ...
              {language === 'ar' ? 'رابط الإعلان / مرجع' : 'Announcement Link / Reference URL'}
            </label>
            <input 
              type="url"
              placeholder="https://..."
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1">
              ...
              {language === 'ar' ? 'رقم مرجع الإعلان' : 'Notice ID / Reference'}
            </label>
            <input 
              type="text"
        ...
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider">
              {language === 'ar' ? 'سجل العروض المستلمة' : 'Bidders Submission Registry'}
            </h3>
          </div>
          <button 
          ...
          >
            ...
            {language === 'ar' ? 'إضافة مقدم عرض' : 'Register New Bidder'}
          </button>
        </div>
        ...
        {showAddBidder && (
          <div className="p-6 bg-gray-50 border-b border-gray-200 animate-in slide-in-from-top duration-300">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-500 uppercase">Bidder Name</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-500 uppercase">Date Received</label>
                <input 
                  type="date" 
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-500 uppercase">Total Offer ({pr.currency})</label>
                <input 
                  type="number" 
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50/50">
                <th className="px-6 py-3 text-left text-[10px] font-bold text-gray-500 uppercase border-b border-gray-200">Bidder Name</th>
                <th className="px-6 py-3 text-left text-[10px] font-bold text-gray-500 uppercase border-b border-gray-200">Date Received</th>
                <th className="px-6 py-3 text-right text-[10px] font-bold text-gray-500 uppercase border-b border-gray-200">Total Offer</th>
                <th className="px-6 py-3 text-center text-[10px] font-bold text-gray-500 uppercase border-b border-gray-200">Status</th>
                <th className="px-6 py-3 text-center text-[10px] font-bold text-gray-500 uppercase border-b border-gray-200">Receipt</th>
                <th className="px-6 py-3 text-right text-[10px] font-bold text-gray-500 uppercase border-b border-gray-200">Actions</th>
              </tr>
            </thead>
              ...
              {announcement.bidders.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center text-sm text-gray-500 font-medium">
                    No bids registered yet for this tender announcement.
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={bidder.id} className="hover:bg-gray-50/30">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                        bidder.status === 'disqualified' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                      }`}>
                        {bidder.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <button 
                        ...
                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded border text-[10px] font-bold transition-colors ${
                          bidder.receiptPrinted 
                            ? 'bg-gray-50 text-gray-400 border-gray-200'
                            : 'bg-white text-blue-600 border-blue-200 hover:bg-blue-50'
                        }`}
                      >
                        <Printer className="w-3 h-3" />
                        {bidder.receiptPrinted ? 'REPRINT' : 'PRINT RECEIPT'}
                      </button>
                    </td>
      ...
      <div className={`p-6 rounded-xl border flex items-start gap-4 ${
        ...
        <div>
          <h4 className={`text-sm font-bold uppercase tracking-wider mb-1 ${isExpired ? 'text-green-900' : 'text-yellow-900'}`}>
            {isExpired ? 'Tender Period Completed' : 'Tender Announcement Period Active'}
          </h4>
          <p className={`text-sm font-medium ${isExpired ? 'text-green-700' : 'text-yellow-700'}`}>
            {isExpired 
              ? 'The announcement period has closed. Bid Analysis (BA/CBA) is now authorized.' 
              : 'Tender announcement period is still open. Bid Analysis (BA) can only be created after the announcement end date.'}
          </p>
        </div>
      </div>
