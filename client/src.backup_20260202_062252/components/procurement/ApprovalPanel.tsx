...
export function ApprovalPanel({ flow, onApprove, onReject, compact = false }: ApprovalPanelProps) {
  ...
  const t = {
    en: {
      ...
      cancel: 'Cancel',
      submit: 'Submit',
      noPermission: 'You do not have permission to approve this step',
    ...
    ar: {
      ...
      rejectLabel: 'سبب الرفض (مطلوب)',
      rejectPlaceholder: 'يرجى تقديم سبب الرفض...',
      commentsLabel: 'التعليقات (اختياري)',
      commentsPlaceholder: 'أضف أي تعليقات إضافية...',
      cancel: 'إلغاء',
      submit: 'إرسال',
      noPermission: 'ليس لديك إذن للموافقة على هذه الخطوة',
  ...
  const handleReject = async () => {
    if (!rejectionReason.trim()) {
      alert(translations.reasonRequired);
      return;
    }
  ...
  return (
    <div className={`bg-white rounded-lg border border-gray-200 ${isRTL ? 'rtl' : 'ltr'}`}>
      <div className="p-4 border-b border-gray-200">
        <h3 className="text-sm font-bold text-gray-900">{translations.title}</h3>
      </div>
              ...
              <div className="flex-1 min-w-0">
                <div className={`flex items-center justify-between gap-2 mb-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <span className={getStatusBadge(step.status)}>
                    {translations[step.status] || translations.notStarted}
                  </span>
                </div>

                {step.approverName && (
                  <div className={`text-xs text-gray-600 ${isRTL ? 'text-right' : 'text-left'}`}>
                    {translations.approver}: {step.approverName}
                  </div>
                )}

                {step.approvedAt && (
                  <div className={`text-xs text-gray-500 ${isRTL ? 'text-right' : 'text-left'}`}>
                    {translations.date}: {new Date(step.approvedAt).toLocaleDateString(language === 'ar' ? 'ar-EG' : 'en-US')}
                  </div>
                )}

                {step.comments && (
                  <div className={`text-xs text-gray-600 mt-1 italic ${isRTL ? 'text-right' : 'text-left'}`}>
                    {step.comments}
                  </div>
                )}
      ...
      {/* Action Buttons */}
      {showActionButtons && (
        <div className={`p-4 border-t border-gray-200 bg-gray-50 flex gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
          <button
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`bg-white rounded-lg shadow-xl max-w-md w-full mx-4 ${isRTL ? 'rtl' : 'ltr'}`}>
            <div className="p-6">
              <h3 className="text-lg font-bold text-gray-900 mb-4">{translations.rejectTitle}</h3>
