...
export const ProjectReportPrintView = forwardRef<HTMLDivElement, ProjectReportPrintViewProps>(function ProjectReportPrintView({
  ...
  const startDate = new Date(reportData.project.startDate);
  const endDate = new Date(reportData.project.endDate);
    ...
    >
      ...
      <div style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #2d5a87 100%)', color: 'white', padding: '12px 16px', marginBottom: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', paddingBottom: '8px', borderBottom: '1px solid rgba(255,255,255,0.2)' }}>
          ...
          <div style={{ textAlign: 'right', fontSize: '8pt' }}>
            <div style={{ fontWeight: 600 }}>Project Report</div>
            <div>{formatDate(new Date().toISOString())}</div>
            <div style={{ opacity: 0.8 }}>Period: {formatDate(reportData.project.startDate)} - {formatDate(reportData.project.endDate)}</div>
          </div>
        </div>
        <div style={{ fontSize: '14pt', fontWeight: 700, marginBottom: '4px' }}>{reportData.project.name}</div>
        <div style={{ fontSize: '9pt', opacity: 0.9 }}>Project Code: {reportData.project.code}</div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', padding: '0 12px', marginBottom: '8px' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <table>
            <tbody>
              <tr><td style={{ fontWeight: 600, width: '35%', background: '#f8fafc' }}>Status</td><td><span style={{ background: '#00a8a8', color: 'white', padding: '1px 6px', borderRadius: '3px', fontSize: '7pt', textTransform: 'uppercase' }}>{reportData.project.status}</span></td></tr>
              ...
              <tr><td style={{ fontWeight: 600, background: '#f8fafc' }}>Days Remaining</td><td style={{ color: '#00a8a8', fontWeight: 600 }}>{reportData.project.daysRemaining} days ({Math.round(timeElapsedPercent)}% elapsed)</td></tr>
            </tbody>
          </table>
        </div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <table>
            <tbody>
              <tr><td style={{ fontWeight: 600, width: '50%', background: '#f8fafc' }}>Implementation Progress</td><td style={{ color: reportData.activities.completionRate === 0 ? '#dc2626' : '#00a8a8', fontWeight: 700 }}>{reportData.activities.completionRate.toFixed(0)}%</td></tr>
              <tr><td style={{ fontWeight: 600, background: '#f8fafc' }}>Budget Burn Rate</td><td style={{ color: reportData.financial.burnRate === 0 ? '#dc2626' : '#1e3a5f', fontWeight: 700 }}>{reportData.financial.burnRate.toFixed(1)}%</td></tr>
            ...
            <tbody>
              {reportData.indicators.details.map((indicator, idx) => (
                <tr key={idx}>
                  ...
                  <td style={{ color: indicator.achievementRate >= 100 ? '#16a34a' : indicator.achievementRate >= 50 ? '#ca8a04' : '#dc2626', fontWeight: 600 }}>{indicator.achievementRate.toFixed(0)}%</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="no-break" style={{ padding: '0 12px', marginBottom: '8px' }}>
        ...
        <table>
          ...
          <tbody>
            <tr>
              <td style={{ fontWeight: 700, color: '#00a8a8', fontSize: '10pt' }}>{formatCurrency(reportData.financial.totalBudget, reportData.project.currency)}</td>
              <td style={{ fontWeight: 700, color: reportData.financial.actualSpent === 0 ? '#dc2626' : '#1e3a5f', fontSize: '10pt' }}>{formatCurrency(reportData.financial.actualSpent, reportData.project.currency)}</td>
              <td style={{ fontWeight: 700, fontSize: '10pt' }}>{formatCurrency(reportData.financial.remaining, reportData.project.currency)}</td>
              <td style={{ fontWeight: 700, color: reportData.financial.burnRate === 0 ? '#dc2626' : '#1e3a5f', fontSize: '10pt' }}>{reportData.financial.burnRate.toFixed(1)}%</td>
            </tr>
          </tbody>
        </table>
