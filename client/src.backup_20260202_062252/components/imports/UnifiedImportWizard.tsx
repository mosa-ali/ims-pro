...
export function UnifiedImportWizard({
  ...
  const t = {
    // Wizard title
    wizardTitle: language === 'en' ? 'Import from Excel' : 'استيراد من Excel',
    
    // Module names
    moduleNames: {
      'activities': language === 'en' ? 'Activities' : 'الأنشطة',
      'tasks': language === 'en' ? 'Tasks Management' : 'إدارة المهام',
      'beneficiaries': language === 'en' ? 'Beneficiaries' : 'المستفيدين',
      'indicators': language === 'en' ? 'Indicators' : 'المؤشرات',
      'case-management': language === 'en' ? 'Case Management' : 'إدارة الحالات',
      'finance-overview': language === 'en' ? 'Finance Overview' : 'نظرة مالية عامة',
      'forecast-plan': language === 'en' ? 'Forecast Plan' : 'خطة التنبؤ',
      'procurement-plan': language === 'en' ? 'Procurement Plan' : 'خطة المشتريات',
      'active-grants': language === 'en' ? 'Active Grants' : 'المنح النشطة',
      'meal': language === 'en' ? 'MEAL' : 'MEAL',
      'chart-of-accounts': language === 'en' ? 'Chart of Accounts' : 'دليل الحسابات',
      'budgets': language === 'en' ? 'Budgets' : 'الميزانيات',
      'expenditures': language === 'en' ? 'Expenditures' : 'النفقات'
    },
    
    // Step 1: File Selection
    selectFile: language === 'en' ? 'Select Excel File' : 'اختر ملف Excel',
    dragDrop: language === 'en' ? 'Drag and drop your Excel file here, or click to browse' : 'اسحب وأفلت ملف Excel هنا، أو انقر للتصفح',
    supportedFormats: language === 'en' ? 'Supported formats: .xlsx, .xls (Max 10MB)' : 'الصيغ المدعومة: .xlsx, .xls (الحد الأقصى 10 ميجابايت)',
    fileSelected: language === 'en' ? 'File selected' : 'تم اختيار الملف',
    changeFile: language === 'en' ? 'Change file' : 'تغيير الملف',
    
    // Step 2: Settings & Preview
    importSettings: language === 'en' ? 'Import Settings' : 'إعدادات الاستيراد',
    fileSummary: language === 'en' ? 'File Summary' : 'ملخص الملف',
    fileName: language === 'en' ? 'File Name' : 'اسم الملف',
    fileSize: language === 'en' ? 'File Size' : 'حجم الملف',
    module: language === 'en' ? 'Module' : 'الوحدة',
    project: language === 'en' ? 'Project' : 'المشروع',
    uploadedBy: language === 'en' ? 'Uploaded by' : 'تم الرفع بواسطة',
    uploadedAt: language === 'en' ? 'Uploaded at' : 'تم الرفع في',
    
    duplicateSettings: language === 'en' ? 'Duplicate Data Settings' : 'إعدادات البيانات المكررة',
    ...
    smartImportLogic: language === 'en' ? 'Smart Import Logic' : 'منطق الاستيراد الذكي',
    ...
    previewSummary: language === 'en' ? 'Preview Summary' : 'ملخص المعاينة',
    totalRows: language === 'en' ? 'Total rows detected' : 'إجمالي الصفوف المكتشفة',
    rowsToInsert: language === 'en' ? 'Rows to insert' : 'الصفوف المراد إدراجها',
    rowsToUpdate: language === 'en' ? 'Rows to update' : 'الصفوف المراد تحديثها',
    rowsToSkip: language === 'en' ? 'Rows to skip' : 'الصفوف المراد تخطيها',
    rowsFailed: language === 'en' ? 'Rows failed validation' : 'الصفوف الفاشلة في التحقق',
    ...
    // Step 3: Results
    importComplete: language === 'en' ? 'Import Complete' : 'اكتمل الاستيراد',
    importSuccess: language === 'en' ? 'Import completed successfully!' : 'اكتمل الاستيراد بنجاح!',
    importId: language === 'en' ? 'Import ID' : 'معرف الاستيراد',
    recordsInserted: language === 'en' ? 'Records inserted' : 'السجلات المدرجة',
    recordsUpdated: language === 'en' ? 'Records updated' : 'السجلات المحدثة',
    recordsSkipped: language === 'en' ? 'Records skipped' : 'السجلات المتخطاة',
    recordsFailed: language === 'en' ? 'Records failed' : 'السجلات الفاشلة',
    viewHistory: language === 'en' ? 'View Import History' : 'عرض سجل الاستيراد',
    
    // Actions
    cancel: language === 'en' ? 'Cancel' : 'إلغاء',
    next: language === 'en' ? 'Next' : 'التالي',
    back: language === 'en' ? 'Back' : 'رجوع',
    finishImport: language === 'en' ? 'Finish Import' : 'إنهاء الاستيراد',
    close: language === 'en' ? 'Close' : 'إغلاق',
    
    // Errors
    fileRequired: language === 'en' ? 'Please select a file' : 'الرجاء اختيار ملف',
    invalidFileType: language === 'en' ? 'Invalid file type. Please upload .xlsx or .xls file' : 'نوع ملف غير صالح. يرجى تحميل ملف .xlsx أو .xls',
    fileTooLarge: language === 'en' ? 'File size exceeds 10MB' : 'حجم الملف يتجاوز 10 ميجابايت',
    uploadFailed: language === 'en' ? 'Upload failed. Please try again.' : 'فشل التحميل. يرجى المحاولة مرة أخرى.'
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="space-y-6">
              <div className={isRTL ? 'text-right' : 'text-left'}>
                ...
                <p className="text-sm text-gray-600">
                  {t.moduleNames[module]} {projectCode && `• ${projectCode}`}
                </p>
              </div>
              ...
              {uploadError && (
                <div className={`bg-red-50 border border-red-200 rounded-lg p-3 flex items-start gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          {currentStep === 'settings-preview' && (
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900 mt-0.5">{t.moduleNames[module]}</p>
                  </div>
                  {projectCode && (
                    <div>
              ...
              {/* Review Message */}
              <div className={`bg-blue-50 border border-blue-200 rounded-lg p-3 flex items-start gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <Info className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
          ...
          {currentStep === 'results' && importResult && (
            <div className="space-y-6">
              {/* Success Banner */}
              <div className={`bg-green-50 border border-green-200 rounded-lg p-4 flex items-start gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
        ...
        {/* Footer Actions */}
        <div className={`border-t p-6 flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
          {currentStep === 'file-selection' && (
            <>
              <button
                onClick={handleClose}
              ...
              <button
                ...
                className={`flex items-center gap-2 px-6 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${isRTL ? 'flex-row-reverse' : ''}`}
              >
                <span>{isProcessing ? (language === 'en' ? 'Processing...' : 'جاري المعالجة...') : t.next}</span>
          ...
          {currentStep === 'settings-preview' && (
            <>
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 rounded-lg transition-colors ${isRTL ? 'flex-row-reverse' : ''}`}
              >
              ...
              >
                {isProcessing ? (language === 'en' ? 'Importing...' : 'جاري الاستيراد...') : t.finishImport}
              </button>
            </>
          )}
