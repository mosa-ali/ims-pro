...
// Local types for QA-specific data structures
interface QASupplier {
 id: string;
 supplierName: string;
 order: number;
}

interface QAItemPrice {
 itemId: string;
 supplierId: string;
 unitPrice: number;
 itemTotal: number;
...
interface QAFinancialScore {
 supplierId: string;
 totalOfferedPrice: number;
 financialScore: number;
}

interface QAFinalScore {
 supplierId: string;
...
// Extended QuotationAnalysis with QA-specific fields
interface ExtendedQA extends QuotationAnalysis {
 prDate?: string;
 ...
 itemPrices: QAItemPrice[];
 technicalCriteria: CBATechnicalCriterion[];
 technicalScores: QATechnicalScore[];
 financialScores: QAFinancialScore[];
 financialWeight?: number;
 finalScores: QAFinalScore[];
 ...
 * Get QA by ID
 */
 getById(id: string): ExtendedQA | null {
 ...
 * Get QA by PR ID
 */
 getByPRId(prId: string): ExtendedQA | null {
 const all = this.getAll();
 ...
 */
 autoCreateFromPR(pr: ProcurementRequest, userId: string): ExtendedQA {
 ...
 * Add supplier to QA
 */
 addSupplier(qaId: string, supplierName: string, userId: string): ExtendedQA | null {
 ...
 * Remove supplier
 */
 removeSupplier(qaId: string, supplierId: string, userId: string): ExtendedQA | null {
 const qa = this.getById(qaId);
 if (!qa || qa.locked) {
 return null;
 }
 ...
 */
 getItemPrice(qa: ExtendedQA, itemId: string, supplierId: string): QAItemPrice | null {
 ...
 */
 recalculateTechnicalScores(qa: ExtendedQA): void {
 ...
 */
 recalculateFinancialScores(qa: ExtendedQA): void {
 ...
 */
 recalculateFinalScores(qa: ExtendedQA): void {
 ...
 */
 submit(qaId: string, userId: string): ExtendedQA | null {
 const qa = this.getById(qaId);
 ...
 * Approve QA
 */
 approve(qaId: string, userId: string): ExtendedQA | null {
 const qa = this.getById(qaId);
 ...
 */
 getSupplierName(qa: ExtendedQA, supplierId: string): string {
