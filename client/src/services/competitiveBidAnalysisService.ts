 ...
 */
 ...
 const bidders: CBABidder[] = evalCriteria.bidders.map((b: any, index: number) => ({
 id: b.id,
 ...
 */
 ...
 // Update bidders
 cba.bidders = evalCriteria.bidders.map((b: any, index: number) => ({
 id: b.id,
 companyName: b.bidderName,
 addedFromCriteria: true,
 order: index + 1
 ...
 cba.technicalScores = cba.bidders.map((bidder: CBABidder) => {
 const existingScore = cba.technicalScores.find((s: CBATechnicalScore) => s.bidderId === bidder.id);
 ...
 cba.financialScores = cba.bidders.map((bidder: CBABidder) => {
 const existingScore = cba.financialScores.find((s: CBAFinancialScore) => s.bidderId === bidder.id);
 return existingScore || {
 bidderId: bidder.id,
 offeredPrice: 0,
 financialScore: 0
 ...
 cba.finalScores = cba.bidders.map((bidder: CBABidder) => {
 const existingScore = cba.finalScores.find((s: CBAFinalScore) => s.bidderId === bidder.id);
 return existingScore || {
 bidderId: bidder.id,
 technicalScore: 0,
 financialScore: 0,
