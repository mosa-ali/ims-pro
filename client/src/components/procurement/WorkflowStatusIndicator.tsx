...
export function WorkflowStatusIndicator({ prId, onRefresh }: Props) {
 ...
 className={`flex items-center gap-2 px-4 py-2 border rounded-lg transition-colors ${ status.nextAction.includes('Manual') ? 'bg-blue-600 text-white border-blue-700 hover:bg-blue-700 cursor-pointer shadow-sm' : 'bg-blue-50 border-blue-200 text-blue-900 cursor-default' }`} data-back-nav>
 {creating ? (
 ...
 <div
 className={`w-12 h-12 rounded-full flex items-center justify-center ${ step.completed ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400' }`}
 >
 <Icon className="w-6 h-6" />
 </div>
 <span
 className={`text-xs font-medium text-center ${ step.completed ? 'text-gray-900' : 'text-gray-500' }`}
 >
 {step.label}
 </span>
 </div>
 ...
 {!isLastStep && (
 <div
 className={`flex-1 h-0.5 mx-2 ${ step.completed ? 'bg-green-600' : 'bg-gray-200' }`}
 />
 )}
 </div>
 );
