...
export function QuestionSettingsPanel({
 ...
 const labels = {
 questionSettings: t.components.questionSettings,
 questionOptions: t.components.questionOptions,
 skipLogic: t.components.skipLogic,
 validation: t.components.validationCriteria,
 save: t.components.save,
 cancel: t.components.cancel,
 required: t.components.required,
 optional: t.components.optional,
 readOnly: t.components.readonly,
 defaultValue: t.components.defaultValue,
 appearance: t.components.appearance,
 addCondition: t.components.addACondition,
 manualXLSForm: t.components.manuallyEnterSkipLogicInXlsform,
 ifQuestion: t.components.ifQuestion,
 operator: t.components.operator,
 value: t.components.value,
 thenAction: t.components.thenAction,
 targetQuestion: t.components.targetQuestion,
 addValidation: t.components.addValidationRule,
 validationType: t.components.validationType,
 errorMessage: t.components.errorMessage,
 choiceList: t.components.choiceList,
 addOption: t.components.addOption,
 };
 ...
 <div className={`w-full max-w-md bg-white h-full shadow-2xl flex flex-col pointer-events-auto ${isRTL ? 'ms-auto' : 'me-auto'}`}>
 {/* Header */}
 <div className={`px-6 py-4 border-b border-gray-200 flex items-center justify-between`}>
 <h2 className={`text-lg font-bold text-gray-900 text-start`}>
 {labels.questionSettings}
 </h2>
 <button
 onClick={onClose}
 ...
 className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${ activeTab === 'options' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-600 hover:text-gray-900' }`}
 >
 {labels.questionOptions}
 </button>
 <button
 onClick={() => setActiveTab('skip_logic')}
 className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${ activeTab === 'skip_logic' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-600 hover:text-gray-900' }`}
 >
 {labels.skipLogic}
 </button>
 <button
 onClick={() => setActiveTab('validation')}
 className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${ activeTab === 'validation' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-600 hover:text-gray-900' }`}
 >
 {labels.validation}
 </button>
 </div>
 ...
 <p className="text-sm font-semibold text-gray-800">{labels.required}</p>
 <p className="text-xs text-gray-500 mt-0.5">
 {t.components.responseIsMandatory}
 </p>
 </div>
 <button
 onClick={() => setLocalQuestion({ ...localQuestion, required: !localQuestion.required })}
 className={`relative w-11 h-6 rounded-full transition-colors ${ localQuestion.required ? 'bg-blue-600' : 'bg-gray-300' }`}
 >
 <span 
 className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform ${ localQuestion.required ? (isRTL ? 'right-0.5' : 'left-5') : (isRTL ? 'right-5' : 'left-0.5') }`}
 />
 </button>
 </div>
 ...
 <div className={`flex items-center justify-between p-4 rounded-lg bg-gray-50 border border-gray-200`}>
 <div className={`text-start`}>
 <p className="text-sm font-semibold text-gray-800">{labels.readOnly}</p>
 <p className="text-xs text-gray-500 mt-0.5">
 {t.components.preventEditingAfterSubmission}
 </p>
 </div>
 <button
 onClick={() => setLocalQuestion({ ...localQuestion, readOnly: !localQuestion.readOnly })}
 className={`relative w-11 h-6 rounded-full transition-colors ${ localQuestion.readOnly ? 'bg-blue-600' : 'bg-gray-300' }`}
 >
 <span 
 className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform ${ localQuestion.readOnly ? (isRTL ? 'right-0.5' : 'left-5') : (isRTL ? 'right-5' : 'left-0.5') }`}
 />
 </button>
 </div>
 ...
 className={`w-full px-4 py-2.5 rounded-lg text-sm bg-white border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${ 'text-start' }`}
 >
 <option value="">{t.components.default}</option>
 {localQuestion.type === 'text' && (
 <>
 <option value="multiline">{t.components.multiline}</option>
 <option value="numbers">{t.components.numbersKeyboard}</option>
 </>
 )}
 {(localQuestion.type === 'select_one' || localQuestion.type === 'select_multiple') && (
 <>
 <option value="minimal">{t.components.dropdown}</option>
 <option value="compact">{t.components.compact}</option>
 <option value="quick">{t.components.quickSelect}</option>
 </>
 )}
 </select>
 </div>
 ...
 className={`flex-1 px-4 py-2 rounded-lg text-sm bg-white border border-gray-300 focus:ring-2 focus:ring-blue-500 ${ 'text-start' }`}
 />
 <button
 onClick={() => {
 ...
 <option value="contains">{t.components.contains}</option>
 </select>
 </div>
 </div>
 ...
 className="w-full px-3 py-2 rounded-md text-sm border border-gray-300 focus:ring-2 focus:ring-blue-500"
 >
 <option value="show">{t.components.showThisQuestion}</option>
 <option value="hide">{t.components.hideThisQuestion}</option>
 <option value="skip">{t.components.skipToQuestion}</option>
 </select>
 </div>
 ...
 <p className="text-sm font-medium text-gray-700 mb-2">{labels.manualXLSForm}</p>
 <textarea
 value={localQuestion.relevant || ''}
 onChange={(e) => setLocalQuestion({ ...localQuestion, relevant: e.target.value })}
 placeholder={t.placeholders.eGAge18AndCountrySyria}
 rows={3}
 ...
 className="w-full px-3 py-2 rounded-md text-sm border border-gray-300 focus:ring-2 focus:ring-blue-500"
 >
 <option value="min">{t.components.minimumValue}</option>
 <option value="max">{t.components.maximumValue}</option>
 <option value="regex">{t.components.patternRegex}</option>
 <option value="date_range">{t.components.dateRange}</option>
 <option value="custom">{t.components.customExpression}</option>
 </select>
 </div>
 ...
 placeholder={t.components.customErrorMessage}
 className="w-full px-3 py-2 rounded-md text-sm border border-gray-300 focus:ring-2 focus:ring-blue-500"
 />
 </div>
 ...
 >
 <span className="font-semibold text-white text-sm">{labels.save}</span>
 </button>
 <button
 onClick={onClose}
