/**
 * Stock Document Workspace
 * 
 * Displays documents organized by the 5-stage stock lifecycle
 * Automatically routes documents as stock movements progress through workflow stages
 * 
 * Stock Lifecycle:
 * 1. Stock Receipts
 * 2. Stock Transfers
...
export function StockDocumentWorkspace() {  const { user } = useAuth();
  const { language, isRTL } = useLanguage();
  const [selectedStage, setSelectedStage] = useState<string | null>(null);

  // Fetch workflow stages with document counts
  const { data: workflowData, isLoading, error } = trpc.stockDocument.getWorkflowStages.useQuery(
    {},
    {
      enabled: !!user,
    }
  ...
  const { data: stageDocuments, isLoading: isLoadingStage } =
    trpc.stockDocument.getByStage.useQuery(
      {
        stage: selectedStage || "01_Stock_Receipts",
      },
      {
  ...
  const translations = {
    en: {
      title: "Stock Document Workflow",
      description: "Documents organized by stock lifecycle stages",
    ...
    ar: {
      title: "سير العمل الوثائقي للمخزون",
      description: "وثائق منظمة حسب مراحل دورة حياة المخزون",
      noDocuments: "لا توجد وثائق في هذه المرحلة",
      documents: "الوثائق",
      stages: "مراحل المخزون",
  ...
  const getStageDisplayName = (stage: string) => {
    const stageNames: Record<string, { en: string; ar: string }> = {
      "01_Stock_Receipts": { en: "Stock Receipts", ar: "إيصالات المخزون" },
      "02_Stock_Transfers": { en: "Stock Transfers", ar: "تحويلات المخزون" },
      "03_Inventory_Reports": { en: "Inventory Reports", ar: "تقارير المخزون" },
      "04_Adjustments": { en: "Adjustments", ar: "التعديلات" },
      "05_Audits": { en: "Audits", ar: "عمليات التدقيق" },
    };
