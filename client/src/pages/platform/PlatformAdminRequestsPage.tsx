...
import { Loader2, CheckCircle, XCircle, Mail, Phone, Briefcase } from 'lucide-react';
import { trpc } from '@/lib/trpc';
import { toast } from 'sonner';

const translations = {
  en: {
    title: 'Platform Admin Access Requests',
    description: 'Review and manage access requests for platform administrator roles',
    noRequests: 'No pending requests',
    noRequestsDesc: 'All access requests have been processed',
  ...
  ar: {
    title: 'طلبات الوصول لمسؤولي المنصة',
    description: 'مراجعة وإدارة طلبات الوصول لأدوار مسؤول المنصة',
    noRequests: 'لا توجد طلبات معلقة',
    noRequestsDesc: 'تم معالجة جميع طلبات الوصول',
...
export default function PlatformAdminRequestsPage() {  const { language } = useLanguage();
  const t = translations[language as keyof typeof translations] || translations.en;
  const [approvingId, setApprovingId] = useState<number | null>(null);
  const [rejectingId, setRejectingId] = useState<number | null>(null);

  // Fetch platform admin requests
  const { data: requests, isLoading, refetch } = trpc.ims.getPlatformAdminRequests.useQuery({});
