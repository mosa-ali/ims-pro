...
// GrantFormModal removed - grants are auto-synced from projects
...
export default function ActiveGrantsPage() {
 ...
 // Grants are auto-synced from projects - no manual create/edit needed
 ...
 // Delete mutation
 const deleteMutation = trpc.grants.delete.useMutation({onSuccess: () => {
 ...
 // Sync from projects mutation
 const syncMutation = trpc.grants.syncFromProjects.useMutation({
 onSuccess: (data) => {
 utils.grants.list.invalidate();
 utils.grants.getDashboardKPIs.invalidate();
 toast.success(data.message || 'Grants synced successfully!');
 },
 onError: (error) => {
 toast.error(error.message || 'Failed to sync grants from projects');
 },
 });

 // Fetch grants from tRPC
 const { data: grants, isLoading, refetch } = trpc.grants.list.useQuery({status: statusFilter,
 ...
 <BackButton label={t.common.backToDonorCrmDashboard} />
 </Link>
 ...
 >
 <RefreshCw className={`w-4 h-4 me-2 ${syncMutation.isPending ? 'animate-spin' : ''}`} />
 {syncMutation.isPending ? 'Syncing...' : 'Sync from Projects'}
 </Button>
 </div>
 </div>
 </div>
 ...
 <Input
 placeholder={t.placeholders.searchByGrantNumberNameOrDonor}
 value={searchTerm}
 ...
 {/* Edit removed - grants are auto-synced from projects */}
 <Button
 variant="outline"
 size="sm"
 title="Delete Grant"
 ...
 : 'Grants are automatically created when you create projects. Go to Programs Management to create a new project.'}
 </p>
 </Card>
 )}

 {/* Grant modals removed - grants are auto-synced from projects */}
