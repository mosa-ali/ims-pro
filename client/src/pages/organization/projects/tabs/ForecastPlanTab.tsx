...
import { ForecastPlanTabSkeleton } from "@/components/ProjectTabSkeletons";
...
const { user } = useAuth();
 ...
 // Use actualSpent directly from budget_items (Financial Overview source of truth)
 ...
 // Budget structure (read-only) - Source of Truth from Financial Overview
 budgetCode: item.budgetCode || '',
 subBL: item.subBudgetLine || '',
 
 // Activity Name (from activities table via LEFT JOIN)
 ...
 // Use project.totalBudget (consistent across all fiscal years)
 ...
 // Use project.spent (source of truth from Financial Overview) instead of summing expenses
 const totalActualSpent = project?.spent ? Number(project.spent) : 0;
 ...
 return <ForecastPlanTabSkeleton />;
 }
 ...
 className={`px-4 py-2 font-medium transition-colors ${ selectedYear === year ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground hover:text-foreground' }`}
 >
 {year}
 </button>
 ))}
 ...
 <p className="ltr-safe text-2xl font-bold text-blue-600 dark:text-blue-400 text-start">
 ...
 <p className="ltr-safe text-2xl font-bold text-purple-600 dark:text-purple-400 text-start">
 ...
 <p className="ltr-safe text-2xl font-bold text-orange-600 dark:text-orange-400 text-start">
 ...
 <p className="ltr-safe text-2xl font-bold text-green-600 dark:text-green-400 text-start">
 ...
 <span className="ltr-safe">{warnings.balanceWarnings.length}</span> {t.forecastPlan.budgetLinesWarning}
 </p>
 </div>
 </div>
 )}
 ...
 <span className="ltr-safe">{warnings.varianceAlerts.length}</span> {t.forecastPlan.budgetLinesExceeded}
 </p>
 </div>
 </div>
 )}
 ...
 <tr>
 <th className={`px-4 py-3 font-medium sticky ${'start-0'} bg-muted z-10 text-start`}>{t.forecastPlan.budgetCode}</th>
 ...
 <th key={i} className="px-4 py-3 text-end font-medium"><span className="ltr-safe">M{i + 1}</span></th>
 ))}
 ...
 <td className="px-4 py-3 font-medium sticky start-0 bg-background">{row.budgetCode}</td>
 ...
 <td className="px-4 py-3 text-end bg-muted">{formatNumber(row.totalBudgetLine)}</td>
 {row.yearNumber > 1 && (
 <td className="px-4 py-3 text-end bg-blue-50 dark:bg-blue-950 font-medium">
 ...
 <td
 key={monthIdx}
 className={`px-4 py-3 text-end ${ !isActive ? 'bg-muted text-muted-foreground' : canEdit ? 'cursor-pointer hover:bg-accent' : '' }`}
 ...
 className="w-full px-2 py-1 text-end border rounded"
 autoFocus
 />
 ) : (
 ...
 <td className="px-4 py-3 text-end font-medium bg-purple-50 dark:bg-purple-950">
 {formatNumber(row.totalForecast)}
 </td>
 <td className="px-4 py-3 text-end font-medium bg-orange-50 dark:bg-orange-950">
 {formatNumber(row.actualSpent)}
 </td>
 <td className="px-4 py-3 text-end font-medium bg-green-50 dark:bg-green-950">
 ...
 {t.forecastPlan.dataPreview} (<span className="ltr-safe">{importPreview.length}</span> {t.forecastPlan.rows})
 </h3>
 <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-x-auto max-h-96">
 <table className="w-full text-sm">
 <thead className="bg-gray-50 dark:bg-gray-900">
 <tr>
 <th className={`px-4 py-2 font-semibold text-start`}>Budget Code</th>
 <th className={`px-4 py-2 font-semibold text-start`}>Budget Item</th>
 <th className="px-4 py-2 text-end font-semibold">M1</th>
 <th className="px-4 py-2 text-end font-semibold">M2</th>
 <th className="px-4 py-2 text-end font-semibold">M3</th>
 <th className="px-4 py-2 text-end font-semibold">...</th>
 <th className="px-4 py-2 text-end font-semibold">Total</th>
 </tr>
 </thead>
 <tbody>
 ...
 <td className="px-4 py-2 text-end">{formatNumber(row.m1)}</td>
 <td className="px-4 py-2 text-end">{formatNumber(row.m2)}</td>
 <td className="px-4 py-2 text-end">{formatNumber(row.m3)}</td>
 <td className="px-4 py-2 text-end">...</td>
 <td className="px-4 py-2 text-end font-semibold">{formatNumber(row.totalForecast)}</td>
 </tr>
 ))}
