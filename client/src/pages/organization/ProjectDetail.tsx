...
// ✅ CRITICAL FIX: Dynamically load project data based on URL project ID
 ...
 <BackButton href="/projects" label={t.projectDetail.backToProjects} />
 ...
 <span className="ltr-safe text-orange-600 font-medium whitespace-nowrap">{daysRemaining} {t.projectDetail.daysRemaining}</span>
 </div>
 </div>
 </div>
 <button className={`project-btn-primary flex-shrink-0`} onClick={handleEditProject}>
 ...
 <Tabs.List className={`project-tabs-list`}>
 {tabs.map((tab) => (
 <Tabs.Trigger
 key={tab.id}
 value={tab.id}
