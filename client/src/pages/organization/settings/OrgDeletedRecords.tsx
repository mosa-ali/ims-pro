...
import { Trash2, RotateCcw, Search, FileText, Leaf, CheckSquare, Square } from 'lucide-react';
...
export default function OrgDeletedRecords() {
 ...
 return new Intl.DateTimeFormat(t.deletedRecords.enus, {
 year: 'numeric',
 month: 'short',
 day: 'numeric',
 hour: '2-digit',
 ...
 const isOrgAdmin = user?.role?.toLowerCase() === 'organization_admin';
 const hasAccess = isPlatformAdmin || isOrgAdmin;

 // Get organization and operating unit IDs from user context
 ...
 onSuccess: () => {
 toast.success(t.deletedRecords.recordRestoredSuccessfully);
 refetch();
 ...
 onSuccess: () => {
 toast.success(t.deletedRecords.recordPermanentlyDeleted);
 refetch();
 ...
 <p className="text-gray-500 font-medium">Organization administrator privileges required.</p>
 <Link href="/organization/settings"><button className="mt-8 px-8 py-3 bg-gray-900 text-white rounded-2xl font-black uppercase tracking-widest text-xs">Return to Settings</button></Link>
 </div>
 </div>
 );
 }
 ...
 const labels = {
 title: t.deletedRecords.organizationAuditVault,
 subtitle: t.deletedRecords.operationalRecoveryOfProjectDataAnd,
 back: t.deletedRecords.back,
 archivedRecords: t.deletedRecords.organizationArchive,
 archivedDesc: 'Records here are specific to your organization and operating units.',
 search: t.deletedRecords.searchRecords,
 filterRecordType: t.deletedRecords.allTypes,
 filterModule: t.deletedRecords.allModules,
 filterDate: t.deletedRecords.dateRange,
 allTime: t.deletedRecords.allHistory,
 last7Days: t.deletedRecords.last7Days,
 last30Days: t.deletedRecords.last30Days,
 last90Days: t.deletedRecords.last90Days,
 recordType: t.deletedRecords.recordType,
 name: t.deletedRecords.recordName,
 module: t.deletedRecords.module,
 deletedBy: t.deletedRecords.deletedBy,
 deletedAt: t.deletedRecords.deletedAt,
 actions: t.deletedRecords.actions,
 restore: t.deletedRecords.restore,
 permanentDelete: t.deletedRecords.deleteForever,
 totalDeleted: t.deletedRecords.totalDeleted,
 canRestore: t.deletedRecords.canRestore,
 canPermanentDelete: t.deletedRecords.canDelete,
 noRecords: t.deletedRecords.noDeletedRecordsFound,
 selectAll: t.deletedRecords.selectAll,
 selected: t.deletedRecords.selected,
 bulkRestore: t.deletedRecords.restoreSelected,
 bulkDelete: t.deletedRecords.deleteSelected,
 };

 const recordTypes = Array.from(new Set(deletedRecords.map((r: any) => r.recordType))).sort();
 const modules = Array.from(new Set(deletedRecords.map((r: any) => r.module).filter(Boolean))).sort();
 ...
 return matchesSearch && matchesRecordType && matchesModule && matchesDate;
 });
 ...
 { label: labels.canRestore, val: stats.canRestore, color: 'text-emerald-600' },
 ...
 <div className="bg-emerald-900 text-white rounded-[32px] p-8 flex items-center gap-6 shadow-xl relative overflow-hidden">
 <div className="absolute end-0 top-0 opacity-10">
 <Leaf className="w-64 h-64 translate-x-20 -translate-y-20" />
 </div>
 ...
 <p className="text-emerald-400 font-medium leading-relaxed">{labels.archivedDesc}</p>
 </div>
 </div>

 {/* Bulk Actions Bar */}
 {selectedRecordIds.length > 0 && (
 <div className="bg-emerald-50 border-2 border-emerald-200 rounded-[24px] p-6 flex items-center justify-between">
 <div className="flex items-center gap-4">
 <div className="p-3 bg-emerald-600 rounded-xl">
 <CheckSquare className="w-5 h-5 text-white" />
 </div>
 <div>
 <p className="font-black text-emerald-900 text-lg">{selectedRecordIds.length} {labels.selected}</p>
 </div>
 </div>
 <div className={`flex items-center gap-3`}>
 <button
 onClick={handleBulkRestore}
 className="px-6 py-3 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-emerald-700 transition-all flex items-center gap-2"
 >
 ...
 className={`w-full bg-gray-50 border-none rounded-2xl py-4 focus:ring-2 focus:ring-emerald-500 font-medium ps-12 pe-4 text-start`}
 />
 </div>
 ...
 className={`w-full px-6 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500 font-black text-xs uppercase tracking-widest text-start`}
 >
 ...
 <th className={`px-10 py-6 text-[10px] font-black text-gray-400 uppercase tracking-[2px] text-start`}>{labels.module}</th>
 ...
 <th className={`px-10 py-6 text-[10px] font-black text-gray-400 uppercase tracking-[2px] text-start`}>{t.deletedRecords.deletionReason}</th>
 ...
 <span className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-lg text-[9px] font-black uppercase tracking-widest">{record.recordType}</span>
 </td>
 <td className={`px-10 py-8 font-black text-gray-900 tracking-tight text-start`}>{record.recordName}</td>
 <td className={`px-10 py-8 text-sm font-bold text-gray-500 text-start`}>{record.module || '-'}</td>
 ...
 {record.deletionReason || (t.deletedRecords.noReasonProvided)}
 </p>
 </div>
 </td>
 <td className={`px-10 py-8 text-start`}>
 ...
 className="px-6 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all flex items-center gap-2"
 >
 ...
 title={t.deletedRecords.permanentlyDeleteRecord}
