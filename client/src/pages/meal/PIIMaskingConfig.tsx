...
export function PIIMaskingConfig() {
 ...
 const labels = {
 title: t.meal.piiMaskingConfiguration,
 subtitle: t.meal.piiMaskingSubtitle,
 saveConfig: t.meal.saveConfiguration,
 
 infoTitle: t.meal.whatIsPiiMasking,
 infoDesc: 'PII (Personally Identifiable Information) masking protects sensitive data by replacing real values with masked versions in exports. This ensures compliance with data protection regulations (GDPR, etc.).',
 
 fieldName: t.meal.fieldName,
 surveyName: t.meal.survey24,
 fieldType: t.meal.type13,
 isPII: t.meal.pii,
 masked: t.meal.masked,
 preview: t.meal.preview,
 
 exampleValue: t.meal.exampleValue,
 maskedValue: t.meal.maskedValue,
 
 selectAll: t.meal.selectAllPiiFields,
 deselectAll: t.meal.deselectAll,
 
 piiDetected: t.meal.piiDetected,
 
 success: t.meal.success,
 configSaved: t.meal.piiMaskingConfigurationSavedSuccessfully,
 };
 ...
 alert(`${labels.configSaved}\n\n${maskedCount} ${t.meal.fieldsWillBeMasked}`);
 };
 ...
 <div className={`flex-1 text-start`}>
 <h3 className="text-sm font-bold text-blue-900 mb-1">{labels.infoTitle}</h3>
 <p className="text-sm text-blue-700">{labels.infoDesc}</p>
 </div>
 </div>
 ...
 <p className="text-sm text-gray-600">{labels.piiDetected}</p>
 ...
 <div className={'text-start'}>
 <p className="text-sm text-gray-600">{labels.masked}</p>
 ...
 <td className={`px-4 py-3 text-sm font-medium text-gray-900 text-start`}>
 {field.fieldName}
 </td>
 <td className={`px-4 py-3 text-sm text-gray-600 text-start`}>
 {field.surveyName}
 </td>
 <td className={`px-4 py-3 text-sm text-gray-600 text-start`}>
 ...
 className={`w-12 h-6 rounded-full transition-colors relative ${ field.isMasked ? 'bg-green-600' : 'bg-gray-300' }`}
 >
 <span
 className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${ field.isMasked ? ('right-1') : ('left-1') }`}
 />
 </button>
 </td>
 ...
 <button
 onClick={handleSave}
 className={`w-full flex items-center justify-center gap-2 rounded-lg py-4 bg-blue-600 hover:bg-blue-700 transition-colors`}
 >
 <Save className="w-5 h-5 text-white" />
 <span className="font-semibold text-white">{labels.saveConfig}</span>
 </button>
 </div>
 </div>
 );
