 ...
 * 
 * FEATURES:
 * - Table view with all surveys (tRPC-backed from database)
...
export function SurveyList() {
 ...
 // ✅ Use tRPC to fetch surveys from database
 const { data: allSurveysData = [], isLoading, error } = trpc.mealSurveys.getAll.useQuery(
 { projectId: projectId ? parseInt(projectId) : undefined },
 { enabled: !!projectId }
 );

 // Transform tRPC data to table format
 useEffect(() => {
 ...
 const tableData: Survey[] = allSurveysData.map((s: any) => ({
 id: String(s.id),
 name: s.title,
 status: s.status === 'published' ? 'published' : s.status === 'archived' ? 'archived' : 'draft',
 owner: s.createdBy || user?.name || 'Unknown',
 ...
 }, [allSurveysData, user?.name, language, t.mealSurvey.enus]);
 ...
 <p className="text-red-900 font-semibold">Error loading surveys</p>
