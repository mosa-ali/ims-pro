...
export function IdentityPersonalCard({
 ...
 <InfoRow icon={User} label={localT.staffId} value={employee.staffId} />
 <InfoRow icon={User} label={localT.fullName} value={employee.fullName} />
 <InfoRow icon={User} label={localT.gender} value={employee.gender} />
 <InfoRow icon={MapPin} label={localT.nationality} value={employee.nationality} />
 <InfoRow icon={Mail} label={localT.email} value={employee.email || '-'} />
 <InfoRow icon={Phone} label={localT.phone} value={employee.phone || '-'} />
 <InfoRow icon={Calendar} label={localT.hireDate} value={formatDate(employee.hireDate)} />
 <InfoRow icon={Building2} label={localT.department} value={employee.department} />
 <InfoRow icon={Briefcase} label={localT.position} value={employee.position} />
 </div>

 {/* Full Address */}
 <div className={`p-4 bg-gray-50 rounded-lg text-start`}>
 <p className="text-xs text-gray-500 mb-1">{localT.fullAddress}</p>
 ...
 <p className="text-xs text-gray-500 mb-1">{localT.identification}</p>
 ...
 {/* CV */}
 <div className={`p-3 border border-gray-200 rounded-lg text-start`}>
 <p className="text-xs text-gray-500 mb-1">{localT.cv}</p>
 <p className="text-sm text-gray-400">
 {t.hrEmployeeCards.noFileUploaded}
 </p>
 </div>
 ...
 <span className="text-sm text-gray-600">{localT.statusBadge}:</span>
 <span className="px-3 py-1 bg-green-50 text-green-700 border border-green-200 rounded text-sm font-medium">
 {employee.status === 'active' ? localT.active : employee.status}
 </span>
 </div>
 </div>
