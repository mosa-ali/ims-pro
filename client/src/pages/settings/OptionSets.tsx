/**
 * Enhanced Option Sets Management
 * 
 * ENHANCEMENTS:
 * - Added type/category support for option values
 * - Added sortOrder for custom ordering
 * - Added color and icon support
 * - Maintains backward compatibility
 * - No design/layout changes from original
 * 
 * PATTERN: Similar to GLOBAL_CURRENCIES in BudgetInformationSection.tsx
 * - Supports bilingual labels (English + Arabic)
 * - Reusable across modules
 * - Database-managed with local helpers
 */
...
export function OptionSets() {
  ...
  const [newValueType, setNewValueType] = useState(''); // NEW: Type/Category support
  const [newValueSortOrder, setNewValueSortOrder] = useState('0'); // NEW: Sort order support
interface OptionSetValue {
  ...
  const addValueMutation = trpc.settings.optionSets.addValue.useMutation({
    onSuccess: () => { toast.success('Value added'); setsQuery.refetch(); setShowAddValue(null); resetValueForm(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const labels = {
    ...
    type: 'Type/Category', // NEW
    sortOrder: 'Sort Order', // NEW
  };

  // Helper function to reset value form
  ...
  // Get all unique types/categories from current option set
  const getValueTypes = (setId: number): string[] => {
    const set = optionSets.find((s: any) => s.id === setId);
    if (!set || !set.values) return [];
    return getUniqueCategories(set.values);
  };
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="border-t border-gray-100 p-4">
                        ...
                        <div className={`flex items-center gap-3 flex-1`}>
                          ...
                          <div className={'text-start flex-1'}>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-gray-400">({val.value})</span>
                              {/* NEW: Display type/category if available */}
                              {val.type && <span className="text-xs px-2 py-0.5 bg-blue-100 text-blue-700 rounded">{val.type}</span>}
                            </div>
                            {/* NEW: Display sort order if available */}
                            {val.sortOrder !== undefined && val.sortOrder > 0 && (
                              <span className="text-xs text-gray-400 mt-1">Sort: {val.sortOrder}</span>
                            )}
                          </div>
                        </div>
                        <div className={`flex items-center gap-2`}>
                  ...
                  {showAddValue === set.id ? (
                    <div className="mt-3 p-3 bg-blue-50 rounded-lg space-y-2">
                      ...
                      {/* NEW: Type/Category input */}
                      <input type="text" value={newValueType} onChange={(e) => setNewValueType(e.target.value)} placeholder={labels.type} className="w-full border border-gray-200 rounded px-3 py-1.5 text-sm" />
                      {/* NEW: Sort order input */}
                      <input type="number" value={newValueSortOrder} onChange={(e) => setNewValueSortOrder(e.target.value)} placeholder={labels.sortOrder} className="w-full border border-gray-200 rounded px-3 py-1.5 text-sm" min="0" />
                      <div className={`flex gap-2`}>
                        <button
                          onClick={() => {
                            if (newValueLabel && newValueKey) {
                              addValueMutation.mutate({
                                optionSetId: set.id,
                                label: newValueLabel,
                                labelAr: newValueLabelAr || undefined,
                                value: newValueKey,
                                type: newValueType || undefined,
                                category: newValueType || undefined,
                                sortOrder: Number(newValueSortOrder) || 0,
                                });
                            }
                          }}
                        ...
                        <button onClick={() => { setShowAddValue(null); resetValueForm(); }} className="px-3 py-1.5 bg-gray-200 text-gray-700 rounded text-sm">{labels.cancel}</button>
                      </div>
                    </div>
                  ) : (
                    <button onClick={() => { setShowAddValue(set.id); resetValueForm(); }} className="mt-3 text-sm text-indigo-600 hover:text-indigo-700 flex items-center gap-1">
      ...
      {/* Create Option Set Modal - UNCHANGED */}
      {showCreateSet && (
