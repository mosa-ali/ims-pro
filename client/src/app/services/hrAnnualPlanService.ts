...
class HRAnnualPlanService {
 ...
 // Create plan from base plan (deep clone)
 createFromBasePlan(
 year: number,
 organization: string,
 basePlanId?: string,
 ...
 // If basePlanId is provided, clone from that plan
 let basePlan: HRAnnualPlan | null = null;
 if (basePlanId) {
 basePlan = this.getById(basePlanId);
 }

 // Calculate year offset for date adjustments
 const yearOffset = basePlan ? year - basePlan.year : 0;

 // Helper to adjust dates by year offset
 ...
 // Clone planned positions with new IDs and adjusted dates
 const clonedPositions: PlannedPosition[] = basePlan?.plannedPositions.map(pos => ({
 ...pos,
 id: uuidv4(),
 plannedStartDate: adjustDate(pos.plannedStartDate),
 plannedEndDate: adjustDate(pos.plannedEndDate)
 })) || [];

 // Clone non-salary costs with new IDs
 const clonedCosts: NonSalaryCost[] = basePlan?.nonSalaryCosts.map(cost => ({
 ...cost,
 id: uuidv4()
 })) || [];

 // Clone training plan with new IDs and adjusted dates
 const clonedTraining: TrainingEntry[] = basePlan?.trainingPlan.map(entry => ({
 ...entry,
 id: uuidv4(),
 plannedDate: adjustDate(entry.plannedDate)
 })) || [];

 // Clone HR risks with new IDs
 const clonedRisks: HRRisk[] = basePlan?.hrRisks.map(risk => ({
 ...risk,
 id: uuidv4()
 })) || [];
 ...
 organization,
 status: 'draft',
 existingWorkforce: [], // Don't clone - should be loaded fresh from employee profiles
 plannedPositions: clonedPositions,
 recruitmentPlan: [], // Will be regenerated
 nonSalaryCosts: clonedCosts,
 trainingPlan: clonedTraining,
 hrRisks: clonedRisks,
 approval: {
 ...
 // Regenerate recruitment plan based on cloned positions
 this.regenerateRecruitmentPlan(newPlan);
 
 // Recalculate metrics
 const finalPlan = this.recalculateMetrics(newPlan);
